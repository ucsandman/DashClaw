 ...
 */
export function getLLMProviderInfo(): { name: string; model: string } | null {
 ...
 */
export async function tryLLMComplete(prompt: string, options: LLMCompleteOptions = {}): Promise<LLMCompleteResult> {
  ...
  try {
    ...
    return { result: null, error: (err as Error)?.message || String(err), provider: provider.name };
  }
}

async function _openaiComplete(provider: DetectedProvider, prompt: string, model: string, maxTokens: number, temperature: number): Promise<LLMCompleteResult> {
...
async function _anthropicComplete(provider: DetectedProvider, prompt: string, model: string, maxTokens: number, temperature: number): Promise<LLMCompleteResult> {
  ...
  const data = await res.json() as { content?: Array<{ text?: string | null }> };
  const text = data.content?.[0]?.text || null;
  return { result: text, error: null, provider: 'anthropic' };
}

async function _googleComplete(provider: DetectedProvider, prompt: string, model: string, maxTokens: number, temperature: number): Promise<LLMCompleteResult> {
  ...
  const data = await res.json() as { candidates?: Array<{ content?: { parts?: Array<{ text?: string | null }> } }> };
...
interface SemanticGuardrailContext {
  action_type?: unknown;
...
export async function checkSemanticGuardrail(
  context: SemanticGuardrailContext,
  instruction: string,
  model: string = getDefaultProviderModel('openai', 'semantic_guard') || 'gpt-4.1-mini'
): Promise<unknown> {
