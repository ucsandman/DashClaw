...
export async function runChecks({ env = process.env, orgId = null } = {}) {
  ...
  // Table names are literal strings matching schema/schema.js; a missing table
  // (partial migration) lands in each block's catch and is surfaced by the
  // database checks instead.
  ...
  try {
    ...
    // degraded column may predate this schema — covered by database checks
  }
  ...
  try {
    ...
    } catch { /* column drift — skip silently */ }
