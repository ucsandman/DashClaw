...
export interface Signal {
  severity: string;
  // Optional so the richer GovernanceSignal (which omits `type`) is assignable;
  // `type` is present at runtime for the signals this email renders, and the
  // read sites (escapeHtml / template) already tolerate an absent value.
  type?: string;
  label: string;
}
