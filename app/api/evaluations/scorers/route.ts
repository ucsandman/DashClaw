...
import { listEvalScorers, createEvalScorer } from '../../../lib/repositories/evaluations.repository';
...
export async function POST(request: Request) {
  try {
    ...
    if (scorer_type === 'llm_judge' && !isLLMAvailable()) {
      (response as Record<string, any>).warning = 'AI provider not configured. This scorer won\'t execute until an AI API key is set (OPENAI_API_KEY, ANTHROPIC_API_KEY, or GOOGLE_AI_API_KEY).';
    }
