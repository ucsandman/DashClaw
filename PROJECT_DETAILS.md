...
As of this verification (2026-07-06), generated API inventory reports **240 routes**: **53 stable**, **22 beta**, **165 experimental**.
...
The canonical Node SDK currently exposes **112 public methods** in `sdk/dashclaw.js` and the Python SDK **196** in `sdk-python/dashclaw/client.py` (both reproducible via `npm run sdk:count` — excludes the constructor and `_`-private methods). The Node surface includes:
...
- model strategies and capability registry/runtime methods
