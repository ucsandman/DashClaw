...
_connect_sse (SSE parsing), _auto_recommend, wait_for_approval,
recommend_action, record_x402_purchase, wrap_client, track, and
report_memory_health BEFORE/AFTER the in-file structural refactor.
...
class AutoRecClient(DashClaw):
    def __init__(self, recommend_result=None, recommend_error=None, guard_result=None,
                 guard_error=None, **overrides):
        kwargs = dict(DEFAULTS)
        kwargs.update(overrides)
        super().__init__(**kwargs)
        self.calls = []
    ...
    def guard(self, context):
        self.guard_contexts.append(context)
        if self.guard_error:
    ...
    def events(self):
        return [
            c["body"]
            for c in self.calls
            if c["path"] == "/api/learning/recommendations/events"
        ]


ACTION_DEF = {"action_type": "api_call", "declared_goal": "g", "risk_score": 50}


def rec_result(confidence=90, rec_id="rec-1", action=None, adapted_fields=None):
    return {
        "action": action or dict(ACTION_DEF),
        "recommendation": {"id": rec_id, "confidence": confidence},
...
class TestAutoRecommend(unittest.TestCase):
    def test_off_mode_passthrough(self):
        client = AutoRecClient(auto_recommend="off")
        out = client._auto_recommend(dict(ACTION_DEF))
        self.assertEqual(
            out, {"action": ACTION_DEF, "recommendation": None, "adapted_fields": []}
        )
        self.assertEqual(client.calls, [])

    def test_missing_action_type_passthrough(self):
        client = AutoRecClient(auto_recommend="enforce")
        out = client._auto_recommend({"declared_goal": "g"})
        self.assertIsNone(out["recommendation"])
        self.assertEqual(out["adapted_fields"], [])

    def test_recommend_failure_passthrough(self):
        client = AutoRecClient(
            auto_recommend="enforce", recommend_error=RuntimeError("down")
        )
        out = client._auto_recommend(dict(ACTION_DEF))
        self.assertEqual(
            out, {"action": ACTION_DEF, "recommendation": None, "adapted_fields": []}
    ...
    def test_no_recommendation_returns_result_unchanged(self):
        result = {"action": dict(ACTION_DEF), "recommendation": None, "adapted_fields": []}
        client = AutoRecClient(auto_recommend="enforce", recommend_result=result)
        self.assertEqual(client._auto_recommend(dict(ACTION_DEF)), result)
        self.assertEqual(client.events(), [])

    def test_low_confidence_overrides(self):
        client = AutoRecClient(
            auto_recommend="enforce", recommend_result=rec_result(confidence=10)
        )
        out = client._auto_recommend(dict(ACTION_DEF))
        action = out["action"]
        self.assertEqual(action["recommendation_id"], "rec-1")
        self.assertFalse(action["recommendation_applied"])
        self.assertTrue(
            action["recommendation_override_reason"].startswith(
                "confidence_below_threshold:"
            )
        )
        events = client.events()
        self.assertEqual(len(events), 1)
        self.assertEqual(events[0]["event_type"], "overridden")
        self.assertEqual(events[0]["recommendation_id"], "rec-1")
        self.assertEqual(events[0]["agent_id"], "agent-1")

    def test_unparseable_confidence_treated_as_zero(self):
        client = AutoRecClient(
            auto_recommend="enforce", recommend_result=rec_result(confidence="nan?!x")
        )
        out = client._auto_recommend(dict(ACTION_DEF))
        self.assertFalse(out["action"]["recommendation_applied"])

    def test_guard_restrictive_overrides(self):
        client = AutoRecClient(
            auto_recommend="enforce",
            recommend_result=rec_result(confidence=95),
            guard_result={"decision": "block"},
        )
        out = client._auto_recommend(dict(ACTION_DEF))
        self.assertEqual(
            out["action"]["recommendation_override_reason"], "guard_restrictive:block"
        )
        self.assertFalse(out["action"]["recommendation_applied"])
        # guard probed with the adapted action context
        self.assertEqual(len(client.guard_contexts), 1)
        self.assertEqual(client.guard_contexts[0]["agent_id"], "agent-1")

    def test_guard_failure_is_tolerated_and_applies(self):
        client = AutoRecClient(
            auto_recommend="enforce",
            recommend_result=rec_result(confidence=95),
        ...
        out = client._auto_recommend(dict(ACTION_DEF))
        self.assertTrue(out["action"]["recommendation_applied"])

    def test_warn_mode_does_not_apply(self):
        client = AutoRecClient(
            auto_recommend="warn",
            recommend_result=rec_result(confidence=95),
            guard_result={"decision": "allow"},
        )
        out = client._auto_recommend(dict(ACTION_DEF))
        self.assertEqual(
            out["action"]["recommendation_override_reason"], "warn_mode_no_autoadapt"
        )
        self.assertFalse(out["action"]["recommendation_applied"])

    def test_enforce_applies_recommendation(self):
        adapted = {**ACTION_DEF, "risk_score": 30}
        client = AutoRecClient(
            auto_recommend="enforce",
            recommend_result=rec_result(
                confidence=95, action=adapted, adapted_fields=["risk_score"]
            ),
            guard_result={"decision": "allow"},
        )
        out = client._auto_recommend(dict(ACTION_DEF))
        action = out["action"]
        self.assertEqual(action["risk_score"], 30)
        self.assertTrue(action["recommendation_applied"])
        self.assertIsNone(action["recommendation_override_reason"])
        self.assertEqual(action["recommendation_id"], "rec-1")
        events = client.events()
        self.assertEqual(len(events), 1)
        self.assertEqual(events[0]["event_type"], "applied")
        self.assertEqual(events[0]["details"]["adapted_fields"], ["risk_score"])

    def test_callback_invoked_and_exceptions_swallowed(self):
        seen = []
...
# ---------------------------------------------------------------------------
# recommend_action: hint application
# ---------------------------------------------------------------------------

class RecommendClient(RecordingDashClaw):
    def __init__(self, recommendations=None, **overrides):
        super().__init__(**overrides)
        self.recommendations = recommendations or []

    def get_recommendations(self, **kwargs):
        self.calls.append({"path": "get_recommendations", "method": "GET", "body": kwargs})
        return {"recommendations": self.recommendations}


class TestRecommendAction(unittest.TestCase):
    def test_non_dict_action_passthrough(self):
        client = RecommendClient()
        out = client.recommend_action("nope")
        self.assertEqual(
            out, {"action": "nope", "recommendation": None, "adapted_fields": []}
        )

    def test_no_recommendations_passthrough(self):
        client = RecommendClient(recommendations=[])
        action = {"action_type": "api_call"}
        out = client.recommend_action(action)
        self.assertEqual(
            out, {"action": action, "recommendation": None, "adapted_fields": []}
        )

    def test_risk_cap_applied_when_above_cap(self):
        rec = {"id": "r1", "hints": {"preferred_risk_cap": 40}}
        client = RecommendClient(recommendations=[rec])
        out = client.recommend_action({"action_type": "t", "risk_score": 80})
        self.assertEqual(out["action"]["risk_score"], 40)
        self.assertEqual(out["adapted_fields"], ["risk_score"])
        self.assertEqual(out["recommendation"], rec)

    def test_risk_cap_applied_when_missing(self):
        rec = {"id": "r1", "hints": {"preferred_risk_cap": 40}}
        client = RecommendClient(recommendations=[rec])
        out = client.recommend_action({"action_type": "t"})
        self.assertEqual(out["action"]["risk_score"], 40)

    def test_risk_cap_not_applied_when_below_cap(self):
        rec = {"id": "r1", "hints": {"preferred_risk_cap": 40}}
        client = RecommendClient(recommendations=[rec])
        out = client.recommend_action({"action_type": "t", "risk_score": 10})
        self.assertEqual(out["action"]["risk_score"], 10)
        self.assertEqual(out["adapted_fields"], [])

    def test_prefer_reversible_only_fills_none(self):
        rec = {"id": "r1", "hints": {"prefer_reversible": True}}
        client = RecommendClient(recommendations=[rec])
        out = client.recommend_action({"action_type": "t"})
        self.assertTrue(out["action"]["reversible"])
        self.assertEqual(out["adapted_fields"], ["reversible"])

        out2 = client.recommend_action({"action_type": "t", "reversible": False})
        self.assertFalse(out2["action"]["reversible"])
        self.assertEqual(out2["adapted_fields"], [])

    def test_confidence_floor_raises_low_confidence(self):
        rec = {"id": "r1", "hints": {"confidence_floor": 70}}
        client = RecommendClient(recommendations=[rec])
        out = client.recommend_action({"action_type": "t", "confidence": 50})
        self.assertEqual(out["action"]["confidence"], 70)
        self.assertEqual(out["adapted_fields"], ["confidence"])

    def test_all_hints_combined(self):
        rec = {
            "id": "r1",
            "hints": {
                "preferred_risk_cap": 30,
                "prefer_reversible": True,
                "confidence_floor": 80,
            },
        }
        client = RecommendClient(recommendations=[rec])
        out = client.recommend_action({"action_type": "t", "risk_score": 99})
        self.assertEqual(
            out["adapted_fields"], ["risk_score", "reversible", "confidence"]
        )

    def test_queries_with_action_type_and_limit_1(self):
        client = RecommendClient(recommendations=[])
        client.recommend_action({"action_type": "deploy"})
        call = client.calls[-1]
        self.assertEqual(call["body"]["action_type"], "deploy")
        self.assertEqual(call["body"]["limit"], 1)
