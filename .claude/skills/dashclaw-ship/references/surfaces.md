...
- `app/page.tsx` — the landing page. **The rendered feature lists are the inline arrays in this file** (the operations-section capability cards ~line 605, plus `corePrimitives`/`frameworkQuickstarts`/`signals` from landingData). The dead-array trap was removed in v2.6d (2026-07-03): `landingData.js` now exports ONLY the three rendered arrays — if you're tempted to add a feature array there, put the copy in `app/page.tsx` instead. After editing, grep `app/page.tsx` for the feature name to prove it renders. CAUTION: the `UseCases` tabs document *different* features — don't conflate.
- `app/landingData.js` — keep its gated counts current (the MCP tool/resource strings are drift-gated). Only `corePrimitives`/`frameworkQuickstarts`/`signals` live here; everything else renders inline in `app/page.tsx`.
