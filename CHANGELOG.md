...
- **`@dashclaw/openclaw-plugin` manifests point at `dist/index.js`** instead of `src/index.ts`. OpenClaw 2026.7.x refuses TypeScript entries for installed packages (source checkouts excepted) — the stale entry broke the gateway's startup plugin repair.
