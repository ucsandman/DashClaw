 ...
 */
export function shouldProbeNow(root = probeRoot(), now = Date.now()) {
    const marker = join(root, '.last-run-openclaw');
    try {
        if (now - statSync(marker).mtimeMs < THROTTLE_MS)
            return false;
    }
    catch {
 ...
 */
function syntheticEvent(witnessPath, runId) {
    return {
        toolName: 'Write',
        params: {
...
export async function runLivenessProbe(options) {
...
async function reportRun(run, options) {
    const base = (options.dashclawUrl || '').replace(/\/+$/, '');
    if (!base || !options.dashclawApiKey)
        return;
    ...
    try {
        ...
        console.warn(`[dashclaw-governance] liveness verdict not reported: ${err instanceof Error ? err.message : 'unknown'}`);
    }
}
