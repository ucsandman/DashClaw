...
export function __resetAutoPairing() {
    attempted = new Set();
}
export function identityKeyPath(agentId) {
...
function directiveTargets(body, agentId) {
...
function apiUrl(config, pathAndQuery) {
    return `${config.dashclawUrl.replace(/\/+$/, '')}${pathAndQuery}`;
}
async function fetchUnreadInbox(config) {
    const res = await fetch(apiUrl(config, `/api/messages?agent_id=${encodeURIComponent(config.agentId)}&direction=inbox&unread=true&limit=50`), { headers: { 'x-api-key': config.dashclawApiKey } });
    if (!res.ok)
        throw new Error(`messages GET failed (${res.status})`);
    const data = (await res.json());
    return Array.isArray(data.messages) ? data.messages : [];
}
async function markRead(config, messageIds) {
    ...
    if (!res.ok)
        throw new Error(`messages PATCH failed (${res.status})`);
}
export async function maybeAutoPair(client, config) {
    if (!config.autoPairing)
        return;
    ...
    try {
        ...
        try {
            ...
            console.warn(`[dashclaw-governance] auto-pairing: submitted pairing ${pairingId} but FAILED to store ` +
        ...
        console.log(`[dashclaw-governance] auto-pairing submitted (${pairingId}) — approve it on /identities. ` +
