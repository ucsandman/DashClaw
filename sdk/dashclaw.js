...
class DashClaw {
  ...
  // ---------------------------------------------------------------------------
  // Drift detection — statistical behavioral-drift monitoring for governed agents.
  // ---------------------------------------------------------------------------

  /**
   * POST /api/drift/alerts (action=detect) — Run drift detection comparing a
   * recent window of agent behavior to the computed baseline.
   * @param {Object} [options]
   * @param {string} [options.agentId] - Scope detection to one agent (default: all)
   * @param {number} [options.windowDays=7] - Recent window to compare against baseline
   * @returns {Promise<Object>}
   */
  async detectDrift({ agentId = null, windowDays = 7 } = {}) {
    return this._post('/api/drift/alerts', { action: 'detect', agent_id: agentId, window_days: windowDays });
  }

  /**
   * POST /api/drift/alerts (action=compute_baselines) — Compute statistical
   * baselines from historical agent data.
   * @param {Object} [options]
   * @param {string} [options.agentId] - Scope baselines to one agent (default: all)
   * @param {number} [options.lookbackDays=30] - History window for the baseline
   * @returns {Promise<Object>}
   */
  async computeDriftBaselines({ agentId = null, lookbackDays = 30 } = {}) {
    return this._post('/api/drift/alerts', { action: 'compute_baselines', agent_id: agentId, lookback_days: lookbackDays });
  }

  /**
   * POST /api/drift/alerts (action=record_snapshots) — Record daily metric
   * snapshots for trend visualization.
   * @returns {Promise<Object>}
   */
  async recordDriftSnapshots() {
    return this._post('/api/drift/alerts', { action: 'record_snapshots' });
  }

  /**
   * GET /api/drift/alerts — List drift alerts with optional filters.
   * @param {Object} [filters]
   * @param {string}  [filters.agentId] - Filter by agent
   * @param {string}  [filters.severity] - Filter by severity
   * @param {boolean} [filters.acknowledged] - Filter by acknowledgement state
   * @param {number}  [filters.limit=50] - Max alerts to return
   * @returns {Promise<Object>}
   */
  async listDriftAlerts({ agentId = null, severity = null, acknowledged = null, limit = 50 } = {}) {
    return this._get('/api/drift/alerts', {
      agent_id: agentId,
      severity,
      acknowledged: acknowledged === null ? null : String(acknowledged),
      limit,
    });
  }

  /**
   * PATCH /api/drift/alerts/:alertId — Acknowledge a drift alert.
   * @param {string} alertId
   * @returns {Promise<Object>}
   */
  async acknowledgeDriftAlert(alertId) {
    return this._patch(`/api/drift/alerts/${alertId}`);
  }

  /**
   * DELETE /api/drift/alerts/:alertId — Delete a drift alert.
   * @param {string} alertId
   * @returns {Promise<Object>}
   */
  async deleteDriftAlert(alertId) {
    return this._delete(`/api/drift/alerts/${alertId}`);
  }

  /**
   * GET /api/drift/stats — Get drift detection statistics.
   * @param {Object} [options]
   * @param {string} [options.agentId] - Scope stats to one agent (default: all)
   * @returns {Promise<Object>}
   */
  async getDriftStats({ agentId = null } = {}) {
    return this._get('/api/drift/stats', { agent_id: agentId });
  }

  /**
   * GET /api/drift/snapshots — Get metric trend snapshots.
   * @param {Object} [options]
   * @param {string} [options.agentId] - Filter by agent
   * @param {string} [options.metric] - Filter by metric name
   * @param {number} [options.limit=30] - Max snapshots to return
   * @returns {Promise<Object>}
   */
  async getDriftSnapshots({ agentId = null, metric = null, limit = 30 } = {}) {
    return this._get('/api/drift/snapshots', { agent_id: agentId, metric, limit });
  }

  /**
   * GET /api/drift/metrics — List available drift detection metrics.
   * @returns {Promise<Object>}
   */
  async getDriftMetrics() {
    return this._get('/api/drift/metrics');
  }

  /**
   * GET /api/actions/assumptions?drift=true — Assumption-drift report: the
   * assumption ledger filtered to drift-relevant rows.
   * @param {Object} [filters] - Optional assumption filters (e.g. agent_id, limit)
   * @returns {Promise<Object>}
   */
  async getDriftReport(filters = {}) {
    return this._get('/api/actions/assumptions', { ...filters, drift: 'true' });
  }
