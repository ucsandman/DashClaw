...
> **Companion file:** [`DESIGN.md`](DESIGN.md) (visual, Stitch format) is a derived view of this document for the impeccable design skill and other DESIGN.md-aware tooling. **This file stays canonical for design** — if they ever diverge, this file wins: update it here first, then re-derive the companion. Product strategy and scope are defined in [`THESIS.md`](THESIS.md).
...
- Semantic color tiers are already formalized — *always use tokens, never hardcode hex*. `text-primary / secondary / tertiary / disabled`, `bg-primary / secondary / tertiary / elevated`, `border / border-hover / border-active`, `status-success / warning / error / info` (plus `-subtle` variants), and the single-prefix status aliases `text- / bg- / border-success | warning | error | info`.
- **Every token accepts a Tailwind opacity modifier** — `border-status-success/20`, `bg-brand/10`, `text-brand/70` — so reach for the tint you actually want rather than working around it. This was silently untrue until 2026-08-10: token colours were plain `var(--color-*)` strings, which Tailwind cannot apply a modifier to, so those classes emitted **no CSS at all** and the element fell back to whatever it inherited. Anything written against the old limitation (inline `style={{ borderColor: … }}`, an arbitrary `border-[color:var(--color-brand)]`, element-level `opacity`) is now a workaround with no reason to exist.
...
_Last updated: 2026-08-10 — token opacity modifiers documented as usable: they compiled to zero CSS until the tokens were made alpha-aware, so 96 tints across the product never rendered (including focus rings, which fell back to Tailwind's default blue). Previously 2026-06-06 — readability/brightness lift: canvas raised off pitch-black with a cool-slate undertone, secondary/tertiary text brightened, small type steps enlarged. (Initial capture 2026-04-10 via `/teach-impeccable`.)_
