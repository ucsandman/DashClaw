...
function registerProviderTools(server: McpServer, store: Store): void {
  ...
  server.registerTool(
    "create_launch_plan",
    {
      ...
      description:
        "Create a stateful launch plan for a project: an ordered step checklist derived from the launch playbook for the declared stack " +
  ...
  server.registerTool(
    "get_launch_status",
    {
      ...
      description:
        "Load a launch plan and report done / pending / blocked-on-approval / failed per step plus THE single next action. Completion is " +
  ...
  server.registerTool(
    "preflight_launch",
    {
      ...
      description:
        "Run before step 1 — verifies the declared stack is actually launchable: provider tokens present AND valid (cheap authenticated " +
  ...
  server.registerTool(
    "verify_launch",
    {
      title: "Verify launch",
      description:
        "Run after the last step — end-to-end verification that the launch actually works: domain resolves, latest deployment READY, " +
