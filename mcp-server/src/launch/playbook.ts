...
interface StepTemplate {
  ...
  toolHint: string;
  /** Every one of these stack items must be declared for the step to appear. */
  requires: LaunchStackItem[];
  /** Step ids this depends on — silently dropped when the dep step is absent. */
  dependsOn: string[];
...
const TEMPLATES: StepTemplate[] = [
  ...
  },
  {
    id: "domain.dns",
    title: ({ domain }) =>
      `Point ${domain ?? "the domain"} at the app (add_vercel_domain for the dnsTarget, then set_dns_records)`,
...
export function validateStack(declared: string[]): LaunchStackItem[] {
  if (!Array.isArray(declared) || declared.length === 0) {
    throw new DashclawError(
      `declared_stack must be a non-empty subset of: ${LAUNCH_STACK_ITEMS.join(", ")}.`,
    );
  }
  ...
  for (const item of declared) {
    if (!(LAUNCH_STACK_ITEMS as readonly string[]).includes(item)) {
      throw new DashclawError(
        `Unknown stack item "${item}". Expected a subset of: ${LAUNCH_STACK_ITEMS.join(", ")}.`,
      );
    }
    if (seen.has(item)) throw new DashclawError(`Duplicate stack item "${item}".`);
    seen.add(item);
  }
