...
export function createLaunchPlan(store: Store, input: CreateLaunchPlanInput): LaunchPlan {
  ...
  if (!environments.some((e) => e.name === environment)) {
    throw new DashclawError(
      `Environment "${environment}" not found for project "${project}" — create it first (add_environment).`,
    );
  }
...
function resolveProjectSlug(store: Store, projectRef?: string): string {
  ...
  if (projectRef) {
    ...
    if (!match) throw new DashclawError(`Project "${projectRef}" not found — create it first (create_project).`);
    return match.slug;
  }
