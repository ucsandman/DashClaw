...
import { DASHCLAW_GATED_TOOLS, governanceEnabled, governanceMisconfigured, } from "./registration.js";
 ...
 */
function registerEmptyPrompts(server) {
...
export function composeServer(server, store) {
    ...
    if (governance) {
        ...
        if (missing) {
            console.error(`[dashclaw] WARNING: governance tools NOT registered — ${missing} is missing while its counterpart is set. ` +
