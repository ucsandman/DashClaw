...
const r2BucketName = (description) => {
    const schema = z
        .string()
        .trim()
        .min(3)
...
const nonNegativeInt = (description) => {
    const schema = z.number().int().nonnegative();
    return description ? schema.describe(description) : schema;
};
const envVarName = (description) => {
    const schema = z
        .string()
        .trim()
        .min(1)
...
export function registerTools(server, store) {
    ...
    }, guard((a) => ({ status: "ok", project: svc.selectProject(store, a.project) })));
    server.registerTool("get_project_context", {
        title: "Get project context",
        description: "THE tool to call FIRST. Returns the full production context for a project/environment: " +
    ...
    server.registerTool("add_environment", {
    ...
    }, guard((a) => ({ status: "ok", environments: svc.listEnvironments(store, a.project) })));
    // --- Provider mappings ---------------------------------------------------
    server.registerTool("map_provider_resource", {
        title: "Map provider resource",
        description: "Bind a provider resource to a project environment. Examples of `resource`: " +
    ...
    server.registerTool("list_provider_mappings", {
        title: "List provider mappings",
        description: "List all environment→provider-resource mappings for a project.",
        inputSchema: { project: optionalNonEmptyString() },
    }, guard((a) => ({ status: "ok", mappings: svc.listProviderMappings(store, a.project) })));
    server.registerTool("get_provider_mapping", {
    ...
    }, guard((a) => ({ status: "ok", connections: svc.listConnections(store, a) })));
    server.registerTool("create_connection", {
        title: "Create provider connection",
        description: "Create an explicit provider connection backed by an environment variable. The secret value is never stored.",
        inputSchema: {
            provider,
    ...
    server.registerTool("set_app_env_vars", {
        title: "Set app env vars",
        description: "Set multiple environment variables on the mapped Vercel or Railway app under one governed env_change action. " +
    ...
    }, guard((a) => pa.setAppEnvVars(store, a)));
    // --- Policy --------------------------------------------------------------
    server.registerTool("check_policy", {
        title: "Check policy",
        description: "Ask whether a capability (read/write/deploy/env_change/delete/destructive_sql/purchase) is " +
    ...
    }, guard((a) => ({ status: "ok", decision: svc.checkPolicy(store, a) })));
    server.registerTool("simulate_action", {
        title: "Simulate action",
        description: "Simulate a provider capability in an environment without executing a provider call or writing audit entries.",
        inputSchema: {
            project: optionalNonEmptyString(),
            environment: nonEmptyString(),
            provider,
    ...
    }, guard((a) => ({ status: "ok", decision: svc.simulateAction(store, a) })));
    server.registerTool("list_policy_rules", {
    ...
    server.registerTool("set_policy_rule", {
        title: "Set policy rule",
        description: "Add an explicit policy rule that overrides defaults. Higher priority wins. Use this to " +
    ...
    }, guard((a) => ({ status: "ok", rule: svc.setPolicyRule(store, a) })));
    ...
    }, guard((a) => ({ status: "ok", ...svc.approveAction(store, a) })));
    ...
    }, guard((a) => ({ status: "ok", ...svc.rejectAction(store, a) })));
    // --- Memory / audit ------------------------------------------------------
    ...
    server.registerTool("write_project_memory", {
        ...
        description: "Save a short note for a project/environment so future agent sessions know what happened " +
    ...
    server.registerTool("explain_action_risk", {
    ...
    }, guard((a) => svc.explainActionRisk(store, a)));
    server.registerTool("governed_action_summary", {
...
function registerProviderTools(server, store) {
    ...
    server.registerTool("get_app_logs", {
        title: "Get app logs",
        description: "Fetch application/deployment logs for a project environment from the mapped provider(s). " +
    ...
    server.registerTool("get_vercel_logs", {
        title: "Get Vercel logs",
        description: "Fetch logs from the mapped Vercel project. If `deployment_id` is given, reads that " +
    ...
    server.registerTool("get_latest_deployment_logs", {
        title: "Get latest deployment logs",
        description: "Convenience: find the latest deployment for the mapped provider (default Vercel) and " +
    ...
    }, guard((a) => pa.latestDeploymentLogs(store, {
        project: a.project,
        environment: a.environment,
        provider: a.provider,
    })));
    // GitHub
    server.registerTool("get_github_repo_context", {
    ...
    }, guard((a) => pa.githubRepoContext(store, a)));
    server.registerTool("get_github_repo_readme", {
        title: "GitHub README",
        description: "Fetch the README of the mapped repo.",
        inputSchema: { project: proj, environment: env },
    }, guard((a) => pa.githubReadme(store, a)));
    server.registerTool("list_github_repo_files", {
    ...
    }, guard((a) => pa.githubListFiles(store, a)));
    server.registerTool("list_github_pull_requests", {
    ...
    }, guard((a) => pa.githubPullRequests(store, a)));
    server.registerTool("list_github_branches", {
    ...
    }, guard((a) => pa.githubBranches(store, a)));
    server.registerTool("get_github_status_checks", {
    ...
    }, guard((a) => pa.githubStatusChecks(store, a)));
    server.registerTool("list_github_workflow_runs", {
    ...
    }, guard((a) => pa.githubWorkflowRuns(store, a)));
    server.registerTool("list_github_workflow_jobs", {
    ...
    }, guard((a) => pa.githubWorkflowJobs(store, a)));
    server.registerTool("rerun_github_workflow_run", {
    ...
    }, guard((a) => pa.githubRerunWorkflowRun(store, a)));
    server.registerTool("cancel_github_workflow_run", {
        ...
        inputSchema: { project: proj, environment: env, runId: positiveInt("GitHub Actions workflow run id") },
    }, guard((a) => pa.githubCancelWorkflowRun(store, a)));
    // Vercel
    server.registerTool("get_vercel_project_context", {
        title: "Vercel project context",
        description: "Read the mapped Vercel project (framework, id).",
        inputSchema: { project: proj, environment: env },
    }, guard((a) => pa.vercelProjectContext(store, a)));
    server.registerTool("get_vercel_deployments", {
        title: "Vercel deployments",
        description: "List recent deployments for the mapped Vercel project.",
        inputSchema: { project: proj, environment: env, limit: positiveInt().optional() },
    }, guard((a) => pa.vercelDeployments(store, a)));
    server.registerTool("get_vercel_deployment_status", {
    ...
    }, guard((a) => pa.vercelDeploymentStatus(store, a)));
    server.registerTool("get_vercel_deployment_logs", {
    ...
    }, guard((a) => pa.vercelDeploymentLogs(store, a)));
    server.registerTool("set_vercel_env_var", {
        title: "Set Vercel env var",
        description: "Set/upsert an environment variable on the mapped Vercel project. PRODUCTION changes " +
    ...
    }, guard((a) => pa.vercelSetEnvVar(store, a)));
    server.registerTool("create_vercel_project", {
        title: "Create Vercel project",
        description: "Create a new Vercel project (optionally with a framework preset). Map it afterwards with " +
    ...
    }, guard((a) => pa.vercelCreateProject(store, a)));
    server.registerTool("add_vercel_domain", {
        title: "Add Vercel domain",
        description: "Attach a domain to a Vercel project. The result includes the DNS record to create at the " +
    ...
    }, guard((a) => pa.vercelAddDomain(store, {
        project: a.project,
        environment: a.environment,
        vercelProject: a.vercel_project,
        domain: a.domain,
    })));
    server.registerTool("create_vercel_deployment", {
    ...
    }, guard((a) => pa.vercelCreateDeployment(store, a)));
    // Railway (GraphQL API)
    server.registerTool("get_railway_project_context", {
        title: "Railway project context",
        description: "Read the mapped Railway project: its name, environments, and services.",
        inputSchema: { project: proj, environment: env },
    }, guard((a) => pa.railwayProjectContext(store, a)));
    server.registerTool("discover_railway_resources", {
        ...
        inputSchema: { project: proj, environment: env },
    }, guard((a) => pa.railwayDiscover(store, a)));
    server.registerTool("get_railway_deployments", {
        ...
        inputSchema: { project: proj, environment: env, limit: positiveInt().optional() },
    }, guard((a) => pa.railwayDeployments(store, a)));
    server.registerTool("get_railway_logs", {
        title: "Get Railway logs",
        description: "Fetch logs from the mapped Railway project. If `deployment_id` is given, reads that " +
            "deployment; otherwise resolves the latest deployment first. Returns the deployment " +
            "id/url/status plus log lines. Read-only and audited.",
        inputSchema: {
            project: proj,
    ...
    server.registerTool("create_railway_deployment", {
        title: "Create Railway deployment",
        description: "Trigger a deployment of the mapped Railway service, or redeploy an existing deployment " +
    ...
    }, guard((a) => pa.railwayCreateDeployment(store, {
        project: a.project,
        environment: a.environment,
        deploymentId: a.deployment_id,
    })));
    server.registerTool("set_railway_env_var", {
        title: "Set Railway variable",
        description: "Create/update a variable on the mapped Railway project/service. PRODUCTION changes " +
    ...
    }, guard((a) => pa.railwaySetEnvVar(store, {
        project: a.project,
        environment: a.environment,
        key: a.key,
        value: a.value,
    ...
    server.registerTool("check_domain_availability", {
        ...
        description: "Check whether domains are available to register, including premium status and pricing. Read-only.",
        inputSchema: {
            project: proj,
            environment: env,
            domains: z.array(nonEmptyString()).min(1).describe("Domain names to check, e.g. [\"example.com\"]"),
        },
    }, guard((a) => pa.checkDomainAvailability(store, a)));
    server.registerTool("list_namecheap_domains", {
    ...
    }, guard((a) => pa.namecheapListDomains(store, {
        project: a.project,
        environment: a.environment,
        page: a.page,
    ...
    server.registerTool("purchase_domain", {
        title: "Purchase domain",
        description: "Register a domain via Namecheap. SPENDS REAL MONEY and ALWAYS requires human approval " +
    ...
    }, guard((a) => pa.purchaseDomain(store, a)));
    server.registerTool("get_dns_records", {
    ...
    }, guard((a) => pa.getDnsRecords(store, a)));
    server.registerTool("set_dns_records", {
        title: "Set DNS records",
        description: "Set the DNS host records for a domain. WARNING: this REPLACES ALL existing host records " +
    ...
    }, guard((a) => pa.setDnsRecords(store, {
        project: a.project,
        environment: a.environment,
        domain: a.domain,
    ...
    // Neon
    server.registerTool("list_neon_projects", {
        title: "List Neon projects",
        description: "List all Neon projects visible to the API key (account-level, read-only).",
        inputSchema: { project: proj, environment: env },
    }, guard((a) => pa.neonListProjects(store, a)));
    server.registerTool("create_neon_project", {
        title: "Create Neon project",
        description: "Provision a new Neon Postgres project. The result includes the connection URI for the " +
    ...
    server.registerTool("get_neon_connection_uri", {
        ...
        description: "Fetch the connection URI (DATABASE_URL) for a Neon project/branch/database/role. The URI " +
    ...
    }, guard((a) => pa.neonGetConnectionUri(store, {
        project: a.project,
        environment: a.environment,
    ...
    // Upstash Redis
    server.registerTool("list_upstash_redis_databases", {
    ...
    }, guard((a) => pa.upstashListRedisDatabases(store, a)));
    server.registerTool("create_upstash_redis_database", {
        title: "Create Upstash Redis database",
        description: "Create an Upstash Redis database and return UPSTASH_REDIS_REST_URL/UPSTASH_REDIS_REST_TOKEN wiring. Production setup requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
            apiHost: optionalNonEmptyString("Optional Upstash API host override"),
    ...
    }, guard((a) => pa.upstashCreateRedisDatabase(store, a)));
    server.registerTool("get_upstash_redis_env", {
        title: "Get Upstash Redis env",
        description: "Return env wiring for UPSTASH_REDIS_REST_URL, UPSTASH_REDIS_REST_TOKEN, and UPSTASH_REDIS_READ_ONLY_REST_TOKEN.",
        inputSchema: {
            project: proj,
            environment: env,
            databaseId: optionalNonEmptyString("Upstash Redis database id; defaults to mapped databaseId"),
        },
    }, guard((a) => pa.upstashGetRedisEnv(store, a)));
    server.registerTool("get_upstash_qstash_env", {
        title: "Get Upstash QStash env",
        description: "Return env wiring for QSTASH_URL, QSTASH_TOKEN, QSTASH_CURRENT_SIGNING_KEY, and QSTASH_NEXT_SIGNING_KEY.",
        inputSchema: {
            project: proj,
            environment: env,
        },
    }, guard((a) => pa.upstashGetQstashEnv(store, a)));
    server.registerTool("list_upstash_qstash_schedules", {
    ...
    }, guard((a) => pa.upstashListQstashSchedules(store, a)));
    server.registerTool("create_upstash_qstash_schedule", {
        title: "Create Upstash QStash schedule",
        description: "Create a QStash cron schedule for an app endpoint. Request bodies and forwarded headers are redacted in QStash; production setup requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
    ...
    }, guard((a) => pa.upstashCreateQstashSchedule(store, a)));
    // Cloudflare R2 object storage
    server.registerTool("list_cloudflare_r2_buckets", {
    ...
    }, guard((a) => pa.cloudflareR2ListBuckets(store, a)));
    server.registerTool("create_cloudflare_r2_bucket", {
        title: "Create Cloudflare R2 bucket",
        description: "Create a Cloudflare R2 bucket and return S3-compatible app env wiring. Production storage setup requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
            accountId: optionalNonEmptyString("Cloudflare account id; defaults to mapped accountId"),
    ...
    }, guard((a) => pa.cloudflareR2CreateBucket(store, a)));
    server.registerTool("get_cloudflare_r2_env", {
        title: "Get Cloudflare R2 env",
        description: "Return R2 app env wiring for R2_ACCOUNT_ID, R2_BUCKET_NAME, R2_ENDPOINT, R2_ACCESS_KEY_ID, and R2_SECRET_ACCESS_KEY.",
        inputSchema: {
            project: proj,
            environment: env,
            accountId: optionalNonEmptyString("Cloudflare account id; defaults to mapped accountId"),
    ...
    }, guard((a) => pa.cloudflareR2GetEnv(store, a)));
    server.registerTool("list_cloudflare_r2_objects", {
    ...
    }, guard((a) => pa.cloudflareR2ListObjects(store, a)));
    // Clerk auth
    server.registerTool("get_clerk_app_env", {
        title: "Get Clerk app env",
        description: "Return client-safe Clerk env wiring for NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY, sign-in/sign-up URLs, and the optional Frontend API URL.",
        inputSchema: {
            project: proj,
            environment: env,
        },
    }, guard((a) => pa.clerkGetAppEnv(store, a)));
    server.registerTool("list_clerk_users", {
    ...
    }, guard((a) => pa.clerkListUsers(store, a)));
    server.registerTool("list_clerk_domains", {
    ...
    }, guard((a) => pa.clerkListDomains(store, a)));
    server.registerTool("list_clerk_redirect_urls", {
    ...
    }, guard((a) => pa.clerkListRedirectUrls(store, a)));
    server.registerTool("create_clerk_redirect_url", {
        title: "Create Clerk redirect URL",
        description: "Whitelist a Clerk redirect URL for OAuth/native auth flows. Production auth setup requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
            url: nonEmptyString("Full redirect URL, e.g. https://app.example.com/callback or my-app://callback"),
        },
    }, guard((a) => pa.clerkCreateRedirectUrl(store, a)));
    // Supabase
    server.registerTool("list_supabase_projects", {
        title: "List Supabase projects",
        description: "List all Supabase projects visible to the access token (account-level, read-only).",
        inputSchema: { project: proj, environment: env },
    }, guard((a) => pa.supabaseListProjects(store, a)));
    server.registerTool("get_supabase_project_context", {
        title: "Supabase project context",
        description: "Get details of the mapped Supabase project (status, region).",
        inputSchema: { project: proj, environment: env },
    }, guard((a) => pa.supabaseProjectContext(store, a)));
    server.registerTool("query_supabase", {
        title: "Query Supabase",
        description: "Run SQL against the mapped Supabase project. Reads run with read_only=true. Destructive SQL " +
    ...
    }, guard((a) => pa.supabaseQuery(store, a)));
    server.registerTool("get_supabase_logs", {
    ...
    }, guard((a) => pa.supabaseLogs(store, a)));
    server.registerTool("apply_supabase_migration", {
        title: "Apply Supabase migration",
        description: "Apply SQL through the Supabase migrations endpoint. Production writes require approval and endpoint access may be restricted by Supabase.",
        inputSchema: { project: proj, environment: env, name: nonEmptyString(), sql: nonEmptyString() },
    }, guard((a) => pa.supabaseApplyMigration(store, a)));
    // Stripe
    server.registerTool("list_stripe_products", {
        ...
        inputSchema: { project: proj, environment: env, limit: positiveInt().optional() },
    }, guard((a) => pa.stripeListProducts(store, a)));
    server.registerTool("list_stripe_customers", {
        ...
        inputSchema: { project: proj, environment: env, limit: positiveInt().optional() },
    }, guard((a) => pa.stripeListCustomers(store, a)));
    ...
    }, guard((a) => pa.stripeListSubscriptions(store, a)));
    ...
    }, guard((a) => pa.stripeListInvoices(store, a)));
    server.registerTool("create_stripe_webhook", {
        title: "Create Stripe webhook",
        description: "Create a webhook endpoint in the environment's Stripe mode. The result includes the whsec_ " +
    ...
    }, guard((a) => pa.stripeCreateWebhook(store, {
        project: a.project,
        environment: a.environment,
        url: a.url,
    ...
    server.registerTool("list_stripe_webhooks", {
        title: "List Stripe webhooks",
        description: "List webhook endpoints in the environment's Stripe mode (signing secrets are never returned by list).",
        inputSchema: { project: proj, environment: env, limit: positiveInt().optional() },
    }, guard((a) => pa.stripeListWebhooks(store, a)));
    server.registerTool("create_stripe_product", {
        title: "Create Stripe product",
        description: "Create a product. Test-mode writes are allowed by default; LIVE-mode writes require approval.",
        inputSchema: { project: proj, environment: env, name: nonEmptyString(), description: z.string().optional() },
    }, guard((a) => pa.stripeCreateProduct(store, a)));
    server.registerTool("create_stripe_price", {
        title: "Create Stripe price",
        description: "Create a price for a product. Test-mode writes allowed by default; LIVE-mode writes require approval.",
        inputSchema: {
            project: proj,
            environment: env,
    ...
    }, guard((a) => pa.stripeCreatePrice(store, a)));
    // Sentry
    server.registerTool("list_sentry_projects", {
    ...
    }, guard((a) => pa.sentryListProjects(store, a)));
    server.registerTool("create_sentry_project", {
        title: "Create Sentry project",
        description: "Create a Sentry observability project. Production observability setup requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
    ...
    }, guard((a) => pa.sentryCreateProject(store, a)));
    server.registerTool("list_sentry_client_keys", {
    ...
    }, guard((a) => pa.sentryListClientKeys(store, a)));
    server.registerTool("create_sentry_client_key", {
        title: "Create Sentry client key",
        description: "Create a Sentry client key and return its public DSN for SENTRY_DSN wiring. Production setup requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
            projectSlug: optionalNonEmptyString("Sentry project slug; defaults to mapped projectSlug"),
    ...
    }, guard((a) => pa.sentryCreateClientKey(store, a)));
    server.registerTool("list_sentry_releases", {
    ...
    }, guard((a) => pa.sentryListReleases(store, a)));
    server.registerTool("create_sentry_release", {
        title: "Create Sentry release",
        description: "Create a Sentry release marker for the mapped project so issues and regressions correlate to a shipped version.",
        inputSchema: {
            project: proj,
            environment: env,
    ...
    }, guard((a) => pa.sentryCreateRelease(store, a)));
    server.registerTool("list_sentry_deploys", {
    ...
    }, guard((a) => pa.sentryListDeploys(store, a)));
    server.registerTool("create_sentry_deploy", {
        title: "Create Sentry deploy",
        description: "Create a Sentry deploy marker for a release/environment. Production deploy markers require approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
            version: nonEmptyString("Release version"),
    ...
    }, guard((a) => pa.sentryCreateDeploy(store, a)));
    // PostHog
    ...
    }, guard((a) => pa.posthogListProjects(store, a)));
    server.registerTool("create_posthog_project", {
        title: "Create PostHog project",
        description: "Create a PostHog analytics project and return NEXT_PUBLIC_POSTHOG_KEY/NEXT_PUBLIC_POSTHOG_HOST wiring. Production setup requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
    ...
    }, guard((a) => pa.posthogCreateProject(store, a)));
    server.registerTool("get_posthog_project_env", {
        title: "Get PostHog project env",
        description: "Return client-safe PostHog env wiring for NEXT_PUBLIC_POSTHOG_KEY, NEXT_PUBLIC_POSTHOG_HOST, and POSTHOG_PROJECT_ID.",
        inputSchema: {
            project: proj,
            environment: env,
            projectId: optionalNonEmptyString("PostHog project id; defaults to mapped projectId"),
        },
    }, guard((a) => pa.posthogGetProjectEnv(store, a)));
    server.registerTool("list_posthog_feature_flags", {
    ...
    }, guard((a) => pa.posthogListFeatureFlags(store, a)));
    server.registerTool("create_posthog_feature_flag", {
        title: "Create PostHog feature flag",
        description: "Create a PostHog feature flag, inactive by default. Production flag writes require approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
            projectId: optionalNonEmptyString("PostHog project id; defaults to mapped projectId"),
    ...
    }, guard((a) => pa.posthogCreateFeatureFlag(store, a)));
    // Resend
    server.registerTool("list_resend_domains", {
        title: "List Resend domains",
        description: "List Resend email domains and their sending/receiving status for the mapped account.",
        inputSchema: { project: proj, environment: env, limit: positiveInt().optional() },
    }, guard((a) => pa.resendListDomains(store, a)));
    server.registerTool("create_resend_domain", {
        title: "Create Resend domain",
        description: "Create a Resend sending domain and return the DNS records to set. Production email/DNS setup requires approval.",
        inputSchema: { project: proj, environment: env, name: nonEmptyString("Domain name, e.g. example.com") },
    }, guard((a) => pa.resendCreateDomain(store, a)));
    server.registerTool("verify_resend_domain", {
    ...
    }, guard((a) => pa.resendVerifyDomain(store, a)));
    server.registerTool("send_resend_email", {
        title: "Send Resend email",
        description: "Send an outbound email through Resend. This is a live external communication and requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
    ...
    }, guard((a) => pa.resendSendEmail(store, a)));
    // Twilio
    server.registerTool("list_twilio_phone_numbers", {
        ...
        inputSchema: { project: proj, environment: env, limit: positiveInt().optional() },
    }, guard((a) => pa.twilioListPhoneNumbers(store, a)));
    server.registerTool("update_twilio_phone_number_webhooks", {
        title: "Update Twilio phone number webhooks",
        description: "Wire a Twilio phone number to your app's inbound SMS and/or voice webhook URLs. " +
    ...
    }, guard((a) => pa.twilioUpdatePhoneNumberWebhooks(store, {
        project: a.project,
        environment: a.environment,
    ...
    server.registerTool("send_twilio_sms", {
        title: "Send Twilio SMS",
        description: "Send an outbound SMS through Twilio. This is a live external communication and requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
    ...
    }, guard((a) => pa.twilioSendSms(store, a)));
    server.registerTool("create_twilio_call", {
        title: "Create Twilio call",
        description: "Create an outbound voice call through Twilio. This is a live external communication and requires approval by default.",
        inputSchema: {
            project: proj,
            environment: env,
            to: nonEmptyString("Recipient phone number in E.164 format"),
