...
function ruleMatches(rule, ctx) {
    const m = rule.match;
    if (m.projectId && m.projectId !== ctx.project.id)
        return false;
    if (m.environmentId && m.environmentId !== ctx.environment.id)
        return false;
    if (m.environmentKind && m.environmentKind !== ctx.environment.kind)
        return false;
    if (m.provider && m.provider !== ctx.provider)
        return false;
    if (m.capability && m.capability !== ctx.capability)
        return false;
    return true;
}
export function evaluatePolicy(rules, ctx) {
    ...
    const resolved = matching.length > 0
        ? {
