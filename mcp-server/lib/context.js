 ...
 */
function effectWord(effect) {
    return effect === "allow" ? "allowed" : effect === "block" ? "blocked" : "approval-required";
}
function decide(store, project, environment, provider, capability, live) {
    const ctx = {
        project,
        environment,
        provider,
...
function actionCatalog(envName, stripeMode) {
    return [
...
function classifyActions(store, project, environment, stripeMode) {
    const buckets = { allowed: [], blocked: [], approvalRequired: [] };
    for (const item of actionCatalog(environment.name, stripeMode)) {
        ...
        if (effect === "allow")
            buckets.allowed.push(item.label);
        else if (effect === "block")
            buckets.blocked.push(item.label);
        else
            buckets.approvalRequired.push(item.label);
    }
    return buckets;
}
async function fetchVercelSnapshot(store, project, environment, projectId, connectionId, teamId) {
    const result = await runGuarded(store, {
        project,
        environment,
        provider: "vercel",
        capability: "read",
        ...
        if (deps.length === 0)
            return { vercelProject: projectId, latest: null };
        const first = deps[0];
        const latest = {
        ...
        if (/error/i.test(latest.state)) {
            try {
                ...
                if (typeof status.errorMessage === "string")
                    latest.errorMessage = status.errorMessage;
            }
            catch {
                /* best-effort */
            }
...
async function buildEnvironmentContext(store, project, env) {
    ...
    const railway = railwayMap && railwayMap.resource.provider === "railway"
        ? {
        ...
        : undefined;
    const githubRepo = githubMap && githubMap.resource.provider === "github"
        ? `${githubMap.resource.owner}/${githubMap.resource.repo}`
        : undefined;
    const supabaseRef = supabaseMap && supabaseMap.resource.provider === "supabase"
        ? supabaseMap.resource.projectRef
        : undefined;
    const stripeMode = stripeMap && stripeMap.resource.provider === "stripe"
    ...
    if (vercelMap && vercelMap.resource.provider === "vercel") {
        const teamId = vercelMap.resource.teamId ??
    ...
    const incidentNote = envMemory.find((m) => m.tags?.includes("incident") || /fail|error|missing|broke/i.test(m.note));
    const lastKnownIssue = vercelSnapshot?.latest?.errorMessage ?? incidentNote?.note;
...
function buildSuggestedActions(buckets, ctx) {
    const out = [];
    ...
    for (const a of buckets.allowed) {
        if (/read|inspect|query|list/i.test(a))
            out.push(a);
    }
    if (ctx.isProduction) {
...
function renderEnvSummary(project, ec) {
    const L = [];
    ...
    if (ec.deployment.latest) {
        ...
        if (ec.deployment.latest.url)
            L.push(`- URL: ${ec.deployment.latest.url}`);
    }
    ...
    if (ec.deployment.lastKnownIssue)
        L.push(`- Last known issue: ${ec.deployment.lastKnownIssue}`);
    if (ec.railway) {
        L.push(`- Railway project: ${ec.railway.projectId}`);
        if (ec.railway.serviceId)
            L.push(`- Railway service: ${ec.railway.serviceId}`);
    }
    L.push("");
    L.push("Database:");
...
export async function buildProjectContext(store, project, envRef) {
    ...
    if (recentAudit.length) {
        summaryParts.push(["", "Recent audit (project):", ...recentAudit
                .slice(0, 5)
    ...
    return {
        ...
        notes: "This is the production context for the project. Resolve project + environment + policy " +
