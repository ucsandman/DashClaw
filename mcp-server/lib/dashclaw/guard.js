...
export function normalizeDashclawDecision(value) {
    if (value === "allow")
        return "allow";
    if (value === "block")
        return "block";
    if (value === "require_approval" || value === "approval_required")
        return "require_approval";
    throw new DashclawError(`Unknown DashClaw decision "${String(value)}".`);
}
export function isRiskyAction(ctx) {
...
function actionType(ctx) {
    if (ctx.capability === "purchase")
        return "provider_purchase";
    if (ctx.provider === "stripe" && ctx.live && ctx.capability === "write")
        return "stripe_live_write";
    if (ctx.provider === "supabase" && ctx.capability === "destructive_sql")
        return "database_destructive_sql";
    if (ctx.provider === "supabase" && ctx.capability === "write")
        return "database_write";
    if (ctx.capability === "deploy")
        return "provider_deploy";
    if (ctx.capability === "env_change")
        return "provider_env_change";
    if (ctx.capability === "delete")
        return "provider_delete";
    if (ctx.capability === "write")
...
function riskScore(ctx) {
    if (ctx.capability === "purchase")
        return 95;
    if (ctx.capability === "destructive_sql" || ctx.capability === "delete")
        return 95;
    if (ctx.live === true)
        return 90;
    if (ctx.capability === "deploy" && ctx.environment.isProduction)
        return 85;
    if (ctx.capability === "env_change" && ctx.environment.isProduction)
        return 85;
    if (ctx.capability === "write" && ctx.environment.isProduction)
        return 80;
    if (ctx.capability === "deploy" || ctx.capability === "env_change")
        return 65;
    if (ctx.capability === "write")
        return 60;
    return 20;
}
function isReversible(ctx) {
    if (ctx.capability === "purchase")
        return false;
    if (ctx.capability === "destructive_sql" || ctx.capability === "delete")
        return false;
    if (ctx.live === true)
        return false;
    if (ctx.environment.isProduction && (ctx.capability === "deploy" || ctx.capability === "env_change"))
        return false;
    return true;
}
...
export function buildDashclawGuardPayload(ctx, localPreview, auditCorrelationId) {
    return {
...
export function localPolicyPreview(store, ctx) {
    return evaluatePolicy(store.data.policyRules, ctx);
}
function objectValue(value, key) {
    return typeof value === "object" && value !== null && key in value ? value[key] : undefined;
}
function stringValue(value) {
    return typeof value === "string" ? value : undefined;
}
export async function guardWithDashclaw(store, ctx, auditCorrelationId = newId("audit")) {
    const payload = buildDashclawGuardPayload(ctx, localPolicyPreview(store, ctx), auditCorrelationId);
    const raw = await dashclawFetch("/api/guard", { method: "POST", body: payload });
