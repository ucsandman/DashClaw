...
import { addEnvironment, createConnection, createProject, dashclawStatus, doctor, ensureDefaultWorkspace, exportAuditLog, exportDashclawEvidence, exportContextSnapshot, getProjectContext, listConnections, listEnvironments, listProjects, mapProviderResource, selectProject, simulateAction, } from "./service.js";
...
function printSetupHelp(serverEntry) {
    const lines = [];
    lines.push("");
    ...
    for (const [name, desc] of REQUIRED_ENV_VARS)
    ...
    switch (cmd) {
        ...
        case "project": {
            if (sub === "create") {
                ...
                if (!name)
                    return failCli("Usage: dashclaw-mcp project create <name>");
                const store = new Store();
        ...
        case "select": {
            if (!sub)
                return failCli("Usage: dashclaw-mcp select <project>");
            const store = new Store();
        ...
        case "env": {
            if (sub === "add") {
                const name = positional[1];
                if (!name)
                    return failCli("Usage: dashclaw-mcp env add <name>");
                const store = new Store();
                print({
                    status: "ok",
        ...
        case "connection": {
            if (sub === "create") {
                ...
                const store = new Store();
                print({ status: "ok", connections: listConnections(store, { provider: flags.provider }) });
            }
            else {
        ...
        case "map": {
            ...
            const store = new Store();
            const resource = { provider, ...parsed };
        ...
        case "audit": {
            ...
            const store = new Store();
            console.log(exportAuditLog(store, {
                project: flags.project,
        ...
        case "dashclaw": {
            ...
            if (sub === "status") {
                ...
                print(exportDashclawEvidence(store, {
                    project: flags.project,
                    environment: flags.env,
                    provider: flags.provider,
                    limit: optionalPositiveInt(flags.limit, "--limit"),
