...
import { resolveProject, resolveEnvironment, requireMapping, findMapping, } from "./resolve.js";
...
function tokenFor(store, provider, connectionId) {
    const conn = findConnection(store, provider, connectionId);
    if (conn)
        return resolveToken(conn);
    if (connectionId) {
    ...
    if (!v || v.trim().length === 0) {
        throw new DashclawError(`No ${provider} connection and ${envVar} is not set. Configure credentials first.`);
    }
    return v.trim();
}
function vercelTeamId(store, mappingTeamId, connectionId) {
    if (mappingTeamId)
        return mappingTeamId;
    const conn = findConnection(store, "vercel", connectionId);
    return conn?.scope?.vercelTeamId ?? process.env.VERCEL_TEAM_ID;
}
function ctx(project, environment, provider, capability, tool, summary, extra) {
...
function assertNonEmptyStringList(name, value) {
    if (value === undefined)
        return undefined;
...
function assertSentryClientKeyUseCase(value) {
    if (value === undefined)
        return undefined;
...
function assertAppEnvVars(value) {
    if (!Array.isArray(value) || value.length === 0) {
...
function assertVercelTargets(value) {
    if (value === undefined)
        return undefined;
...
// --- GitHub ----------------------------------------------------------------
export async function githubRepoContext(store, input) {
    ...
    return runGuarded(store, ctx(project, environment, "github", "read", "get_github_repo_context", `repo ${label}`, {
        resourceLabel: label,
    }), () => gh.getRepoContext(tokenFor(store, "github", m.connectionId), r.owner, r.repo));
}
export async function githubReadme(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo}`;
    return runGuarded(store, ctx(project, environment, "github", "read", "get_github_repo_readme", `readme ${label}`, {
        resourceLabel: label,
    }), () => gh.getReadme(tokenFor(store, "github", m.connectionId), r.owner, r.repo));
}
export async function githubListFiles(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo}`;
    return runGuarded(store, ctx(project, environment, "github", "read", "list_github_repo_files", `files ${label}`, {
        resourceLabel: label,
    }), () => gh.listFiles(tokenFor(store, "github", m.connectionId), r.owner, r.repo, input.path ?? ""));
}
// --- Vercel ----------------------------------------------------------------
function vercelResource(store, project, environment) {
    const m = requireMapping(store, project, environment, "vercel");
    return { ...m.resource, connectionId: m.connectionId };
}
export async function vercelProjectContext(store, input) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "vercel", "read", "get_vercel_project_context", `project ${r.projectId}`, {
...
export async function vercelDeployments(store, input) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "vercel", "read", "get_vercel_deployments", `deployments ${r.projectId}`, {
        resourceLabel: r.projectId,
    }), () => {
...
export async function vercelListEnvVars(store, input) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "vercel", "read", "get_vercel_env_var_names", `env var names ${r.projectId}`, {
        resourceLabel: r.projectId,
    }), () => vc.listEnvVarNames(tokenFor(store, "vercel", r.connectionId), r.projectId, vercelTeamId(store, r.teamId, r.connectionId)));
}
export async function githubPullRequests(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo}`;
    return runGuarded(store, ctx(project, environment, "github", "read", "list_github_pull_requests", `pull requests ${label}`, {
        resourceLabel: label,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function githubBranches(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo}`;
    return runGuarded(store, ctx(project, environment, "github", "read", "list_github_branches", `branches ${label}`, {
        resourceLabel: label,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function githubStatusChecks(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const ref = assertNonEmptyString("ref", input.ref);
    const label = `${r.owner}/${r.repo}@${ref}`;
    return runGuarded(store, ctx(project, environment, "github", "read", "get_github_status_checks", `status checks ${label}`, {
        resourceLabel: label,
    }), () => gh.getCombinedStatus(tokenFor(store, "github", m.connectionId), r.owner, r.repo, ref));
}
export async function githubWorkflowRuns(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo}`;
    return runGuarded(store, ctx(project, environment, "github", "read", "list_github_workflow_runs", `workflow runs ${label}`, {
        resourceLabel: label,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function githubWorkflowJobs(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo} run ${input.runId}`;
    return runGuarded(store, ctx(project, environment, "github", "read", "list_github_workflow_jobs", `workflow jobs ${label}`, {
        resourceLabel: label,
    }), () => {
...
export async function githubRerunWorkflowRun(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo} run ${input.runId}`;
    return runGuarded(store, ctx(project, environment, "github", "write", "rerun_github_workflow_run", `rerun workflow run ${label}`, {
        resourceLabel: label,
    }), () => {
        assertPositiveInteger("runId", input.runId);
...
export async function githubCancelWorkflowRun(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "github");
    const r = m.resource;
    const label = `${r.owner}/${r.repo} run ${input.runId}`;
    return runGuarded(store, ctx(project, environment, "github", "write", "cancel_github_workflow_run", `cancel workflow run ${label}`, {
        resourceLabel: label,
    }), () => {
        assertPositiveInteger("runId", input.runId);
...
export async function vercelDeploymentStatus(store, input) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "vercel", "read", "get_vercel_deployment_status", `status ${input.deploymentId}`, {
...
export async function vercelDeploymentLogs(store, input) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "vercel", "read", "get_vercel_deployment_logs", `logs ${input.deploymentId}`, {
        resourceLabel: input.deploymentId,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function vercelSetEnvVar(store, input) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    const target = input.target ?? [environment.isProduction ? "production" : "preview"];
    return runGuarded(store, ctx(project, environment, "vercel", "env_change", "set_vercel_env_var", `set env ${input.key} on ${r.projectId}`, {
        resourceLabel: `${r.projectId}:${input.key}`,
    }), () => vc.setEnvVar(tokenFor(store, "vercel", r.connectionId), r.projectId, { key: assertNonEmptyString("key", input.key), value: input.value, target }, vercelTeamId(store, r.teamId, r.connectionId)));
}
export async function vercelCreateDeployment(store, input) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    const target = environment.isProduction ? "production" : "preview";
    return runGuarded(store, ctx(project, environment, "vercel", "deploy", "create_vercel_deployment", `deploy ${r.projectId} (${target})`, {
        resourceLabel: r.projectId,
    }), () => vc.createDeployment(tokenFor(store, "vercel", r.connectionId), {
 ...
 */
export async function vercelCreateProject(store, input) {
    const { project, environment } = resolve(store, input);
    ...
    return runGuarded(store, ctx(project, environment, "vercel", "write", "create_vercel_project", `create Vercel project ${name}`, {
        resourceLabel: name,
    }), () => vc.createProject(tokenFor(store, "vercel", mapping?.connectionId), { name, framework: input.framework }, vercelTeamId(store, teamId, mapping?.connectionId)));
}
/**
 ...
 */
export async function vercelAddDomain(store, input) {
    const { project, environment } = resolve(store, input);
    ...
    const mapping = findMapping(store, environment, "vercel");
    const teamId = mapping?.resource.provider === "vercel" ? mapping.resource.teamId : undefined;
    return runGuarded(store, ctx(project, environment, "vercel", "write", "add_vercel_domain", `attach ${domain} to Vercel project ${vercelProject}`, {
        resourceLabel: `${vercelProject}:${domain}`,
    }), () => vc.addProjectDomain(tokenFor(store, "vercel", mapping?.connectionId), vercelProject, domain, vercelTeamId(store, teamId, mapping?.connectionId)));
}
// --- App logs --------------------------------------------------------------
//
...
const LOG_PROVIDERS = ["vercel", "railway"];
/**
...
function redactSecrets(text) {
    ...
    return text
        ...
        .replace(/\b([A-Z0-9_]*(?:SECRET|TOKEN|PASSWORD|API_?KEY|PRIVATE_KEY)[A-Z0-9_]*)\s*[=:]\s*("?)[^\s"]+\2/gi, "$1=***REDACTED***");
}
function normalizeVercelEvent(e) {
    const level = e.type === "stderr" || e.type === "error" ? "error" : "info";
    const timestamp = typeof e.created === "number" && e.created > 0 ? new Date(e.created).toISOString() : "";
    return { level, timestamp, message: redactSecrets(e.text ?? "") };
}
function normalizeRailwayLog(l) {
...
function assertValidSince(since) {
    if (!since)
        return;
 ...
 */
async function fetchVercelLogsData(token, r, teamId, opts) {
    ...
    if (!deploymentId) {
        ...
        try {
            ...
            if (typeof status.readyState === "string")
                deployment_status = status.readyState;
            if (typeof status.url === "string")
                deployment_url = `https://${status.url}`;
        }
        catch {
    ...
    try {
        ...
        const limitation = logs.length === 0
        ...
        return {
            ...
            time_range,
            logs: [],
            limitation: `Could not fetch deployment logs (${err instanceof Error ? err.message : String(err)}). ` +
...
function runVercelLogs(store, input, tool) {
    const { project, environment } = resolve(store, input);
    const r = vercelResource(store, project, environment);
    const teamId = vercelTeamId(store, r.teamId, r.connectionId);
    const label = input.deploymentId ?? r.projectId;
    return runGuarded(store, ctx(project, environment, "vercel", "read", tool, `logs ${label}`, { resourceLabel: label }), () => fetchVercelLogsData(tokenFor(store, "vercel", r.connectionId), r, teamId, {
        deploymentId: input.deploymentId,
        since: input.since,
        limit: input.limit,
    }));
...
export function vercelLogs(store, input) {
    return runVercelLogs(store, input, "get_vercel_logs");
}
// --- Railway (logs) --------------------------------------------------------
function railwayResource(store, project, environment) {
 ...
 */
async function fetchRailwayLogsData(token, r, opts) {
    assertPositiveInteger("limit", opts.limit);
    ...
    let deployment_status;
    if (!deploymentId) {
        const deps = await rw.listDeployments(token, { projectId: r.projectId, environmentId: r.environmentId, serviceId: r.serviceId }, 1);
        if (deps.length === 0) {
            return {
                resource: { project: r.projectId },
                time_range,
    ...
    try {
        ...
        return {
            ...
            time_range,
            logs: [],
            limitation: `Could not fetch Railway deployment logs (${err instanceof Error ? err.message : String(err)}). ` +
                "Returning the deployment status only.",
            audit_written: true,
        };
    }
...
function runRailwayLogs(store, input, tool) {
    const { project, environment } = resolve(store, input);
    const r = railwayResource(store, project, environment);
    const label = input.deploymentId ?? r.projectId;
    return runGuarded(store, ctx(project, environment, "railway", "read", tool, `logs ${label}`, { resourceLabel: label }), () => fetchRailwayLogsData(tokenFor(store, "railway", r.connectionId), r, {
        deploymentId: input.deploymentId,
        since: input.since,
        limit: input.limit,
    }));
...
export function railwayLogs(store, input) {
    return runRailwayLogs(store, input, "get_railway_logs");
}
/** get_railway_project_context — Railway project + its environments/services. */
export async function railwayProjectContext(store, input) {
    const { project, environment } = resolve(store, input);
    const r = railwayResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "railway", "read", "get_railway_project_context", `project ${r.projectId}`, {
        resourceLabel: r.projectId,
...
export async function railwayDeployments(store, input) {
    const { project, environment } = resolve(store, input);
    const r = railwayResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "railway", "read", "get_railway_deployments", `deployments ${r.projectId}`, {
        resourceLabel: r.projectId,
    }), () => {
        assertPositiveInteger("limit", input.limit);
        return rw.listDeployments(tokenFor(store, "railway", r.connectionId), { projectId: r.projectId, environmentId: r.environmentId, serviceId: r.serviceId }, input.limit ?? 10);
    });
}
export async function railwayDiscover(store, input) {
    const { project, environment } = resolve(store, input);
    const mapping = findMapping(store, environment, "railway");
    return runGuarded(store, ctx(project, environment, "railway", "read", "discover_railway_resources", "discover railway projects"), () => rw.listProjects(tokenFor(store, "railway", mapping?.connectionId)));
}
/**
 ...
 */
export async function railwayCreateDeployment(store, input) {
    const { project, environment } = resolve(store, input);
    const r = railwayResource(store, project, environment);
    const label = input.deploymentId ?? r.projectId;
    return runGuarded(store, ctx(project, environment, "railway", "deploy", "create_railway_deployment", `deploy ${label}`, {
        resourceLabel: label,
    }), () => {
        const token = tokenFor(store, "railway", r.connectionId);
        if (input.deploymentId)
            return rw.redeploy(token, input.deploymentId);
        if (!r.environmentId || !r.serviceId) {
            throw new DashclawError("Railway deploy needs the mapping to include environmentId and serviceId " +
 ...
 */
export async function railwaySetEnvVar(store, input) {
    const { project, environment } = resolve(store, input);
    const r = railwayResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "railway", "env_change", "set_railway_env_var", `set var ${input.key} on ${r.projectId}`, {
        resourceLabel: `${r.projectId}:${input.key}`,
    }), () => {
        if (!r.environmentId) {
            throw new DashclawError("Railway variable changes need the mapping to include environmentId.");
        }
...
export async function setAppEnvVars(store, input) {
    const { project, environment } = resolve(store, input);
    ...
    if (targetProvider === "vercel") {
        ...
        return runGuarded(store, ctx(project, environment, "vercel", "env_change", "set_app_env_vars", summary, {
            ...
            for (const item of vars) {
                await vc.setEnvVar(tokenFor(store, "vercel", r.connectionId), r.projectId, { key: item.key, value: item.value, target }, vercelTeamId(store, r.teamId, r.connectionId));
            }
    ...
    const r = railwayResource(store, project, environment);
    const summary = `set ${summaryKeys.length} env vars on ${r.projectId}: ${envKeySummary(summaryKeys)}`;
    return runGuarded(store, ctx(project, environment, "railway", "env_change", "set_app_env_vars", summary, {
        resourceLabel: `${r.projectId}:${envKeySummary(summaryKeys)}`,
    }), async () => {
        const vars = assertAppEnvVars(input.vars);
        const keys = vars.map((item) => item.key);
...
export async function checkDomainAvailability(store, input) {
    const { project, environment } = resolve(store, input);
    ...
    return runGuarded(store, ctx(project, environment, "namecheap", "read", "check_domain_availability", `check availability of ${label}`, {
        resourceLabel: label,
    }), () => nc.checkDomains(tokenFor(store, "namecheap"), input.domains));
}
/** list_namecheap_domains — domains in the Namecheap account (read-only). */
export async function namecheapListDomains(store, input) {
    const { project, environment } = resolve(store, input);
    return runGuarded(store, ctx(project, environment, "namecheap", "read", "list_namecheap_domains", "list Namecheap domains"), () => {
 ...
 * approval round-trip.
 */
export async function purchaseDomain(store, input) {
    const { project, environment } = resolve(store, input);
    const domain = assertNonEmptyString("domain", input.domain);
    const registrant = loadRegistrantContact(store.paths.config);
    if (!registrant) {
        throw new DashclawError(`Domain purchase needs a registrant contact. Add a "namecheap.registrant" block to ` +
    ...
    return runGuarded(store, ctx(project, environment, "namecheap", "purchase", "purchase_domain", `purchase domain ${domain}`, {
        live: true,
...
export async function getDnsRecords(store, input) {
    const { project, environment } = resolve(store, input);
    const domain = assertNonEmptyString("domain", input.domain);
    return runGuarded(store, ctx(project, environment, "namecheap", "read", "get_dns_records", `DNS records for ${domain}`, {
        resourceLabel: domain,
 ...
 */
export async function setDnsRecords(store, input) {
    const { project, environment } = resolve(store, input);
    const domain = assertNonEmptyString("domain", input.domain);
    if (!Array.isArray(input.records) || input.records.length === 0) {
        throw new DashclawError("records must be a non-empty list — setHosts REPLACES ALL host records, so an empty list would wipe the domain's DNS.");
    }
...
export async function neonListProjects(store, input) {
    const { project, environment } = resolve(store, input);
    return runGuarded(store, ctx(project, environment, "neon", "read", "list_neon_projects", "list Neon projects"), () => ne.listProjects(tokenFor(store, "neon")));
}
/** create_neon_project — provision a Neon project (capability "write"). */
export async function neonCreateProject(store, input) {
    const { project, environment } = resolve(store, input);
    const label = input.name ?? "(default name)";
    return runGuarded(store, ctx(project, environment, "neon", "write", "create_neon_project", `create Neon project ${label}`, {
        resourceLabel: label,
 ...
 */
export async function neonGetConnectionUri(store, input) {
    const { project, environment } = resolve(store, input);
    const id = assertNonEmptyString("neonProjectId", input.neonProjectId);
    return runGuarded(store, ctx(project, environment, "neon", "read", "get_neon_connection_uri", `connection URI for Neon project ${id}`, {
        resourceLabel: id,
...
// --- Upstash Redis -----------------------------------------------------------
function upstashCredentials(store, connectionId) {
...
function upstashResource(store, project, environment) {
...
function assertUpstashPlatform(value) {
...
function assertQstashMethod(value) {
    if (value === undefined)
        return undefined;
...
export async function upstashListRedisDatabases(store, input) {
    const { project, environment } = resolve(store, input);
    return runGuarded(store, ctx(project, environment, "upstash", "read", "list_upstash_redis_databases", "list Upstash Redis databases"), () => {
...
export async function upstashCreateRedisDatabase(store, input) {
    const { project, environment } = resolve(store, input);
    const databaseName = assertNonEmptyString("databaseName", input.databaseName);
    return runGuarded(store, ctx(project, environment, "upstash", "env_change", "create_upstash_redis_database", `create Upstash Redis database ${databaseName}`, {
...
export async function upstashGetRedisEnv(store, input) {
    const { project, environment } = resolve(store, input);
    ...
    return runGuarded(store, ctx(project, environment, "upstash", "read", "get_upstash_redis_env", `get Upstash Redis env wiring for ${r.databaseId}`, {
...
export async function upstashGetQstashEnv(store, input) {
    const { project, environment } = resolve(store, input);
    const r = upstashResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "upstash", "read", "get_upstash_qstash_env", "get QStash env wiring", {
...
export async function upstashListQstashSchedules(store, input) {
    const { project, environment } = resolve(store, input);
    const r = upstashResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "upstash", "read", "list_upstash_qstash_schedules", "list QStash schedules", {
        resourceLabel: "qstash",
...
export async function upstashCreateQstashSchedule(store, input) {
    const { project, environment } = resolve(store, input);
    const r = upstashResource(store, project, environment);
    ...
    return runGuarded(store, ctx(project, environment, "upstash", "env_change", "create_upstash_qstash_schedule", `create QStash schedule ${scheduleId ?? destination}`, { resourceLabel: scheduleId ?? "qstash-schedule" }), () => {
...
function cloudflareR2Resource(store, project, environment, accountId) {
    const mapped = findMapping(store, environment, "cloudflare_r2");
    if (!mapped && accountId === undefined) {
        throw new DashclawError(`No cloudflare_r2 mapping for ${project.slug}/${environment.name}. Add one with map_provider_resource.`);
    }
    const resource = (mapped?.resource ?? { provider: "cloudflare_r2", accountId });
    return {
        ...resource,
...
function assertCloudflareR2BucketName(value, field = "bucketName") {
...
function assertCloudflareR2Jurisdiction(value) {
    if (value === undefined)
        return undefined;
...
function cloudflareR2BucketName(r, inputBucketName) {
...
function cloudflareR2AppCredentials(r) {
...
export async function cloudflareR2ListBuckets(store, input) {
    const { project, environment } = resolve(store, input);
    const r = cloudflareR2Resource(store, project, environment, input.accountId);
    return runGuarded(store, ctx(project, environment, "cloudflare_r2", "read", "list_cloudflare_r2_buckets", `list R2 buckets for ${r.accountId}`, {
...
export async function cloudflareR2CreateBucket(store, input) {
    const { project, environment } = resolve(store, input);
    const r = cloudflareR2Resource(store, project, environment, input.accountId);
    ...
    return runGuarded(store, ctx(project, environment, "cloudflare_r2", "env_change", "create_cloudflare_r2_bucket", `create R2 bucket ${bucketName}`, {
...
export async function cloudflareR2GetEnv(store, input) {
    const { project, environment } = resolve(store, input);
    const r = cloudflareR2Resource(store, project, environment, input.accountId);
    const bucketName = cloudflareR2BucketName(r, input.bucketName);
    return runGuarded(store, ctx(project, environment, "cloudflare_r2", "read", "get_cloudflare_r2_env", `get R2 env wiring for ${bucketName}`, {
        resourceLabel: `${r.accountId}/${bucketName}`,
...
export async function cloudflareR2ListObjects(store, input) {
    const { project, environment } = resolve(store, input);
    const r = cloudflareR2Resource(store, project, environment, input.accountId);
    const bucketName = cloudflareR2BucketName(r, input.bucketName);
    return runGuarded(store, ctx(project, environment, "cloudflare_r2", "read", "list_cloudflare_r2_objects", `list R2 objects in ${bucketName}`, {
        resourceLabel: `${r.accountId}/${bucketName}`,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
// --- Clerk ------------------------------------------------------------------
function clerkResource(store, project, environment) {
...
function assertClerkRedirectUrl(value) {
...
export async function clerkGetAppEnv(store, input) {
    const { project, environment } = resolve(store, input);
    const r = clerkResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "clerk", "read", "get_clerk_app_env", "get Clerk app env wiring", {
        resourceLabel: "app",
...
export async function clerkListUsers(store, input) {
    const { project, environment } = resolve(store, input);
    const r = clerkResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "clerk", "read", "list_clerk_users", "list Clerk users", {
        resourceLabel: "users",
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function clerkListDomains(store, input) {
    const { project, environment } = resolve(store, input);
    const r = clerkResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "clerk", "read", "list_clerk_domains", "list Clerk domains", {
...
export async function clerkListRedirectUrls(store, input) {
    const { project, environment } = resolve(store, input);
    const r = clerkResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "clerk", "read", "list_clerk_redirect_urls", "list Clerk redirect URLs", {
...
export async function clerkCreateRedirectUrl(store, input) {
    const { project, environment } = resolve(store, input);
    const r = clerkResource(store, project, environment);
    const url = assertClerkRedirectUrl(input.url);
    return runGuarded(store, ctx(project, environment, "clerk", "env_change", "create_clerk_redirect_url", "create Clerk redirect URL", {
        resourceLabel: "redirect_url",
    }), () => ck.createRedirectUrl(tokenFor(store, "clerk", r.connectionId), { apiHost: r.apiHost, url }));
}
// --- Sentry -----------------------------------------------------------------
function sentryResource(store, project, environment) {
...
function sentryProjectSlug(r, inputProjectSlug) {
...
function sentryProjectSlugs(r, inputProjects) {
...
export async function sentryListProjects(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "sentry", "read", "list_sentry_projects", `list Sentry projects for ${r.organizationSlug}`, {
...
export async function sentryCreateProject(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    const name = assertNonEmptyString("name", input.name);
    const teamSlug = input.teamSlug ?? r.teamSlug;
    return runGuarded(store, ctx(project, environment, "sentry", "env_change", "create_sentry_project", `create Sentry project ${name}`, {
        resourceLabel: r.organizationSlug,
...
export async function sentryListClientKeys(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    const projectSlug = sentryProjectSlug(r, input.projectSlug);
    return runGuarded(store, ctx(project, environment, "sentry", "read", "list_sentry_client_keys", `list Sentry client keys for ${r.organizationSlug}/${projectSlug}`, {
...
export async function sentryCreateClientKey(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    const projectSlug = sentryProjectSlug(r, input.projectSlug);
    return runGuarded(store, ctx(project, environment, "sentry", "env_change", "create_sentry_client_key", `create Sentry client key for ${r.organizationSlug}/${projectSlug}`, {
        resourceLabel: `${r.organizationSlug}/${projectSlug}`,
    }), () => {
...
export async function sentryListReleases(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "sentry", "read", "list_sentry_releases", `list Sentry releases for ${r.organizationSlug}`, {
        resourceLabel: r.organizationSlug,
    }), () => {
        const query = input.query === undefined ? undefined : assertNonEmptyString("query", input.query);
...
export async function sentryCreateRelease(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    const version = assertNonEmptyString("version", input.version);
    return runGuarded(store, ctx(project, environment, "sentry", "write", "create_sentry_release", `create Sentry release ${version}`, {
...
export async function sentryListDeploys(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    const version = assertNonEmptyString("version", input.version);
    return runGuarded(store, ctx(project, environment, "sentry", "read", "list_sentry_deploys", `list Sentry deploys for ${version}`, {
        resourceLabel: `${r.organizationSlug}/${version}`,
...
export async function sentryCreateDeploy(store, input) {
    const { project, environment } = resolve(store, input);
    const r = sentryResource(store, project, environment);
    const version = assertNonEmptyString("version", input.version);
    const deployEnvironment = assertNonEmptyString("deployEnvironment", input.deployEnvironment);
    return runGuarded(store, ctx(project, environment, "sentry", "deploy", "create_sentry_deploy", `create Sentry deploy marker ${version} to ${deployEnvironment}`, {
...
// --- PostHog ----------------------------------------------------------------
function posthogResource(store, project, environment) {
...
function posthogProjectId(r, inputProjectId) {
...
function assertPostHogActive(value) {
    if (value === undefined)
        return undefined;
...
function assertPostHogFlagType(value) {
    if (value === undefined)
        return undefined;
...
export async function posthogListProjects(store, input) {
    const { project, environment } = resolve(store, input);
    const r = posthogResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "posthog", "read", "list_posthog_projects", `list PostHog projects for ${r.organizationId}`, {
...
export async function posthogGetProjectEnv(store, input) {
    const { project, environment } = resolve(store, input);
    const r = posthogResource(store, project, environment);
    const projectId = posthogProjectId(r, input.projectId);
    const hosts = ph.resolveHosts(r.apiHost, r.ingestHost);
    return runGuarded(store, ctx(project, environment, "posthog", "read", "get_posthog_project_env", `get PostHog env wiring for ${projectId}`, {
...
export async function posthogCreateProject(store, input) {
    const { project, environment } = resolve(store, input);
    const r = posthogResource(store, project, environment);
    const name = assertNonEmptyString("name", input.name);
    const hosts = ph.resolveHosts(r.apiHost, r.ingestHost);
    return runGuarded(store, ctx(project, environment, "posthog", "env_change", "create_posthog_project", `create PostHog project ${name}`, {
        resourceLabel: r.organizationId,
    }), async () => {
        const created = await ph.createProject(tokenFor(store, "posthog", r.connectionId), r.organizationId, {
            ...
            name,
            productDescription: input.productDescription === undefined ? undefined : assertNonEmptyString("productDescription", input.productDescription),
...
export async function posthogListFeatureFlags(store, input) {
    const { project, environment } = resolve(store, input);
    const r = posthogResource(store, project, environment);
    const projectId = posthogProjectId(r, input.projectId);
    return runGuarded(store, ctx(project, environment, "posthog", "read", "list_posthog_feature_flags", `list PostHog feature flags for ${projectId}`, {
        resourceLabel: `${r.organizationId}/${projectId}`,
    }), () => {
        assertPositiveInteger("limit", input.limit);
        const search = input.search === undefined ? undefined : assertNonEmptyString("search", input.search);
...
export async function posthogCreateFeatureFlag(store, input) {
    const { project, environment } = resolve(store, input);
    const r = posthogResource(store, project, environment);
    const projectId = posthogProjectId(r, input.projectId);
    const key = assertNonEmptyString("key", input.key);
    return runGuarded(store, ctx(project, environment, "posthog", "write", "create_posthog_feature_flag", `create PostHog feature flag ${key}`, {
...
// --- Resend -----------------------------------------------------------------
function resendResource(store, project, environment) {
...
export async function resendListDomains(store, input) {
    const { project, environment } = resolve(store, input);
    const r = resendResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "resend", "read", "list_resend_domains", `list Resend domains for ${r.domain}`, {
...
export async function resendCreateDomain(store, input) {
    const { project, environment } = resolve(store, input);
    const r = resendResource(store, project, environment);
    const name = assertNonEmptyString("name", input.name);
    return runGuarded(store, ctx(project, environment, "resend", "env_change", "create_resend_domain", `create Resend domain ${name}`, {
        resourceLabel: name,
...
export async function resendVerifyDomain(store, input) {
    const { project, environment } = resolve(store, input);
    const r = resendResource(store, project, environment);
    const domainId = assertNonEmptyString("domainId", input.domainId);
    return runGuarded(store, ctx(project, environment, "resend", "env_change", "verify_resend_domain", `verify Resend domain ${r.domain}`, {
        resourceLabel: r.domain,
...
export async function resendSendEmail(store, input) {
    const { project, environment } = resolve(store, input);
    const r = resendResource(store, project, environment);
    const from = input.from ?? r.defaultFrom;
    return runGuarded(store, ctx(project, environment, "resend", "write", "send_resend_email", "send outbound email via Resend", {
        live: true,
        resourceLabel: r.domain,
    }), () => {
        if (!from) {
...
// --- Twilio -----------------------------------------------------------------
function twilioResource(store, project, environment) {
...
export async function twilioListPhoneNumbers(store, input) {
    const { project, environment } = resolve(store, input);
    const r = twilioResource(store, project, environment);
    return runGuarded(store, ctx(project, environment, "twilio", "read", "list_twilio_phone_numbers", `list Twilio phone numbers for ${r.accountSid}`, {
...
export async function twilioUpdatePhoneNumberWebhooks(store, input) {
    const { project, environment } = resolve(store, input);
    const r = twilioResource(store, project, environment);
    ...
    return runGuarded(store, ctx(project, environment, "twilio", "env_change", "update_twilio_phone_number_webhooks", `update Twilio phone number webhooks for ${phoneNumberSid}`, { resourceLabel: phoneNumberSid }), () => {
        if (!smsUrl && !voiceUrl) {
...
export async function twilioSendSms(store, input) {
    const { project, environment } = resolve(store, input);
    const r = twilioResource(store, project, environment);
    ...
    return runGuarded(store, ctx(project, environment, "twilio", "write", "send_twilio_sms", "send outbound SMS via Twilio", {
        live: true,
        resourceLabel: r.accountSid,
    }), () => {
        if (!from && !messagingServiceSid) {
            throw new DashclawError("send_twilio_sms requires either from, messagingServiceSid, or a mapped Twilio fromNumber/messagingServiceSid.");
        }
...
export async function twilioCreateCall(store, input) {
    const { project, environment } = resolve(store, input);
    const r = twilioResource(store, project, environment);
    const from = input.from ?? r.fromNumber;
    return runGuarded(store, ctx(project, environment, "twilio", "write", "create_twilio_call", "create outbound voice call via Twilio", {
        live: true,
        resourceLabel: r.accountSid,
    }), () => {
        if (!from) {
...
export async function latestDeploymentLogs(store, input) {
    ...
    const { project, environment } = resolve(store, input);
    return runGuarded(store, ctx(project, environment, provider, "read", "get_latest_deployment_logs", `latest logs (${provider})`), async () => ({
        resource: { provider },
        time_range: {},
        logs: [],
 ...
 */
export async function appLogs(store, input) {
    const { project, environment } = resolve(store, input);
    ...
    if (targets.length === 0) {
        return {
            ...
            providers: [],
            limitation: "No mapped providers support log fetching for this environment. Map a Vercel or " +
    ...
    for (const p of targets) {
        if (p === "vercel") {
            ...
            providers.push(await runGuarded(store, ctx(project, environment, p, "read", "get_app_logs", `logs (${p})`), async () => ({
                resource: { provider: p },
...
// --- Supabase --------------------------------------------------------------
export async function supabaseListProjects(store, input) {
    const { project, environment } = resolve(store, input);
    ...
    return runGuarded(store, ctx(project, environment, "supabase", "read", "list_supabase_projects", "list supabase projects"), () => sb.listProjects(tokenFor(store, "supabase", mapping?.connectionId)));
}
export async function supabaseProjectContext(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "supabase");
    const r = m.resource;
    return runGuarded(store, ctx(project, environment, "supabase", "read", "get_supabase_project_context", `project ${r.projectRef}`, {
        resourceLabel: r.projectRef,
    }), () => sb.getProject(tokenFor(store, "supabase", m.connectionId), r.projectRef));
}
export async function supabaseQuery(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "supabase");
    const r = m.resource;
    const classified = classifySql(input.sql);
    return runGuarded(store, ctx(project, environment, "supabase", classified.capability, "query_supabase", `SQL (${classified.keyword}) on ${r.projectRef}`, { resourceLabel: r.projectRef }), 
...
export async function supabaseLogs(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "supabase");
    const r = m.resource;
    return runGuarded(store, ctx(project, environment, "supabase", "read", "get_supabase_logs", `logs ${r.projectRef}`, {
        resourceLabel: r.projectRef,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function supabaseApplyMigration(store, input) {
    const { project, environment } = resolve(store, input);
    const m = requireMapping(store, project, environment, "supabase");
    const r = m.resource;
    return runGuarded(store, ctx(project, environment, "supabase", "write", "apply_supabase_migration", `migration ${input.name} on ${r.projectRef}`, {
        resourceLabel: r.projectRef,
...
// --- Stripe ----------------------------------------------------------------
function stripeMode(store, environment) {
    const m = findMapping(store, environment, "stripe");
    if (m && m.resource.provider === "stripe")
        return m.resource.mode;
    // Fall back to env kind if no explicit mapping.
    return environment.isProduction ? "live" : "test";
}
function stripeKeyFor(store, environment, mode) {
    const m = findMapping(store, environment, "stripe");
    if (m?.connectionId)
        return tokenFor(store, "stripe", m.connectionId);
    return resolveStripeKey(mode);
}
export async function stripeListProducts(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "read", "list_stripe_products", `list products (${mode})`, {
        resourceLabel: mode,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function stripeListPrices(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "read", "list_stripe_prices", `list prices (${mode})`, {
        resourceLabel: mode,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function stripeListCustomers(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "read", "list_stripe_customers", `list customers (${mode})`, {
        resourceLabel: mode,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function stripeListSubscriptions(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "read", "list_stripe_subscriptions", `list subscriptions (${mode})`, {
        resourceLabel: mode,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function stripeListInvoices(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "read", "list_stripe_invoices", `list invoices (${mode})`, {
        resourceLabel: mode,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function stripeCreateProduct(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "write", "create_stripe_product", `create product "${input.name}" (${mode})`, {
        live: mode === "live",
        resourceLabel: mode,
 ...
 */
export async function stripeCreateWebhook(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    const url = assertNonEmptyString("url", input.url);
    return runGuarded(store, ctx(project, environment, "stripe", "write", "create_stripe_webhook", `create webhook endpoint for ${url} (${mode})`, {
        live: mode === "live",
        resourceLabel: mode,
    }), () => {
...
export async function stripeListWebhooks(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "read", "list_stripe_webhooks", `list webhook endpoints (${mode})`, {
        resourceLabel: mode,
    }), () => {
        assertPositiveInteger("limit", input.limit);
...
export async function stripeCreatePrice(store, input) {
    const { project, environment } = resolve(store, input);
    const mode = stripeMode(store, environment);
    return runGuarded(store, ctx(project, environment, "stripe", "write", "create_stripe_price", `create price for ${input.product} (${mode})`, {
        live: mode === "live",
        resourceLabel: mode,
    }), () => {
