...
export const TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        description: 'Check a submitted plan: overall status (pending | approved | partially_approved | denied | ' +
...
export function createToolHandlers(client) {
    ...
    const confidenceKey = (input) => `${agentId(input)}|${input.action_type}|${input.declared_goal}`;
    function rememberConfidence(input, confidence) {
    ...
    function takeConfidence(input) {
        const key = confidenceKey(input);
        const entry = pendingConfidence.get(key);
        if (!entry)
            return undefined;
        pendingConfidence.delete(key);
        if (Date.now() - entry.at > CONFIDENCE_TTL_MS)
            return undefined;
    ...
    return {
        ...
        async dashclaw_plan_submit(input) {
        ...
        async dashclaw_plan_status(input) {
