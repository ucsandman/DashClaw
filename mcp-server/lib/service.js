...
export function createProject(store, input) {
    ...
    if (!name)
        throw new DashclawError("Project name must be a non-empty string.");
    const slug = slugify(input.slug ?? name);
    if (!slug)
        throw new DashclawError("Project name/slug produced an empty slug.");
...
export function selectProject(store, projectRef) {
...
export function addEnvironment(store, input) {
    const name = input.name.trim();
    if (!name) {
        throw new DashclawError("Environment name must be a non-empty string.");
    }
    if (input.kind !== undefined)
        assertEnvironmentKind(input.kind);
    const project = resolveProject(store, input.project);
    const kind = input.kind ?? KIND_BY_NAME[name.toLowerCase()] ?? "development";
    if (store.data.environments.some((e) => e.projectId === project.id && e.name === name)) {
        throw new DashclawError(`Environment "${name}" already exists for project "${project.slug}".`);
    }
    const env = {
        id: newId("env"),
...
export function listEnvironments(store, projectRef) {
    const project = resolveProject(store, projectRef);
    return store.data.environments.filter((e) => e.projectId === project.id);
}
export function getProjectContext(store, projectRef, environment) {
    const project = resolveProject(store, projectRef);
...
function validateProviderResource(provider, resource) {
    if (!resource || typeof resource !== "object") {
        throw new DashclawError("Invalid provider resource: expected an object.");
    }
    if (resource.provider !== provider) {
        throw new DashclawError(`Resource provider "${resource.provider}" does not match "${provider}".`);
    }
    switch (provider) {
        case "github":
            requireNonEmptyString(resource.owner, "github.owner");
            requireNonEmptyString(resource.repo, "github.repo");
            return;
        case "vercel":
            requireNonEmptyString(resource.projectId, "vercel.projectId");
            return;
        case "supabase":
            requireNonEmptyString(resource.projectRef, "supabase.projectRef");
            return;
        case "stripe": {
        ...
        case "railway":
            requireNonEmptyString(resource.projectId, "railway.projectId");
            return;
        case "upstash":
            requireNonEmptyString(resource.databaseId, "upstash.databaseId");
            return;
        case "cloudflare_r2":
            requireNonEmptyString(resource.accountId, "cloudflare_r2.accountId");
            return;
        case "sentry":
            requireNonEmptyString(resource.organizationSlug, "sentry.organizationSlug");
            return;
        case "posthog":
            requireNonEmptyString(resource.organizationId, "posthog.organizationId");
            return;
        case "resend":
            requireNonEmptyString(resource.domain, "resend.domain");
            return;
        case "twilio":
            requireNonEmptyString(resource.accountSid, "twilio.accountSid");
            return;
        case "clerk":
            requireNonEmptyString(resource.publishableKey, "clerk.publishableKey");
            return;
    }
}
function assertPolicyEffect(effect) {
...
function assertCapability(capability) {
    const capabilities = ["read", "write", "deploy", "env_change", "delete", "destructive_sql", "purchase"];
    if (typeof capability !== "string" || !capabilities.includes(capability)) {
        throw new DashclawError(`Invalid policy capability "${String(capability)}". Expected one of: ${capabilities.join(", ")}.`);
    }
}
function assertEnvironmentKind(kind) {
    const kinds = ["development", "staging", "production"];
    if (typeof kind !== "string" || !kinds.includes(kind)) {
        throw new DashclawError(`Invalid environment kind "${String(kind)}". Expected one of: ${kinds.join(", ")}.`);
    }
}
function validatePolicyRuleInput(input) {
    ...
    if (match.provider !== undefined)
        assertProviderId(match.provider);
    if (match.capability !== undefined)
        assertCapability(match.capability);
    if (match.environmentKind !== undefined)
        assertEnvironmentKind(match.environmentKind);
    if (match.projectId !== undefined)
        requireNonEmptyString(match.projectId, "policy.match.projectId");
    if (match.environmentId !== undefined)
        requireNonEmptyString(match.environmentId, "policy.match.environmentId");
}
export function ensureConnection(store, provider, opts) {
...
export function createConnection(store, input) {
...
export function listConnections(store, input = {}) {
...
export function mapProviderResource(store, input) {
    assertProviderId(input.provider);
    ...
    if (input.connectionId !== undefined) {
        ...
        if (connection.provider !== input.provider) {
            throw new DashclawError(`Connection "${connectionId}" is for ${connection.provider}, not ${input.provider}.`);
        }
    }
    ...
    store.update((s) => {
        ...
        s.mappings = s.mappings.filter((m) => !(m.environmentId === environment.id && m.provider === input.provider));
        s.mappings.push({
            id,
            projectId: project.id,
...
export function listProviderMappings(store, projectRef) {
    const project = resolveProject(store, projectRef);
    const envName = (id) => store.data.environments.find((e) => e.id === id)?.name ?? id;
...
export function getProviderMapping(store, input) {
    const project = resolveProject(store, input.project);
    const environment = resolveEnvironment(store, project, input.environment);
...
// Policy
// ---------------------------------------------------------------------------
export function checkPolicy(store, input) {
    assertProviderId(input.provider);
...
export function simulateAction(store, input) {
    assertProviderId(input.provider);
    assertCapability(input.capability);
    const project = resolveProject(store, input.project);
    const environment = resolveEnvironment(store, project, input.environment);
...
export function setPolicyRule(store, input) {
...
function requirePendingApproval(store, approvalId) {
...
function approvalAuditContext(store, approval) {
...
function appendApprovalAudit(store, approval, tool, result, note) {
    ...
    store.appendAudit({
        ...
        provider: "core",
        tool,
        actionSummary: `${tool === "approve_action" ? "approved" : "rejected"} ${approval.id}: ${approval.actionSummary}`,
        policyDecision: "n/a",
        result,
        errorMessage: note,
...
export function listPendingApprovals(store, input = {}) {
    let projectId;
    if (input.project)
        projectId = resolveProject(store, input.project).id;
...
export function approveAction(store, input) {
...
export function rejectAction(store, input) {
    const approval = requirePendingApproval(store, input.approvalId);
    const note = input.note?.trim();
    if (input.note !== undefined && !note) {
...
// Memory + audit
// ---------------------------------------------------------------------------
export function writeProjectMemory(store, input) {
    const note = input.note.trim();
    if (!note)
        throw new DashclawError("Project memory note must be a non-empty string.");
...
export function readProjectMemory(store, input) {
    const project = resolveProject(store, input.project);
    const environmentId = input.environment
        ? resolveEnvironment(store, project, input.environment).id
        : undefined;
...
function envVarCheckId(provider) {
    return `env.${provider}`;
}
function envVarSuffix(envVar) {
    return envVar.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "credential";
}
function hasEnvVar(envVar) {
    return typeof process.env[envVar] === "string" && process.env[envVar].trim().length > 0;
}
function stripeCredentialEnvVar(resource) {
    if (resource?.provider === "stripe" && resource.mode === "live")
        return "STRIPE_LIVE_SECRET_KEY";
    return "STRIPE_TEST_SECRET_KEY";
}
function credentialEnvVars(provider, resource, connection) {
    ...
    if (provider === "upstash") {
        ...
        if (resource?.provider === "upstash")
            envVars.push(resource.qstashTokenEnvVar ?? "QSTASH_TOKEN");
    }
...
function globalCredentialEnvVars(provider) {
...
function addCredentialChecks(checks, provider, envVars, idFor) {
...
function connectionForMapping(store, provider, connectionId) {
    if (connectionId)
...
export function doctor(store, input = {}) {
    const checks = [];
    checks.push({
        id: "storage.home",
        status: "pass",
    ...
    for (const env of environments) {
        ...
        for (const provider of PROVIDER_IDS) {
            ...
            addCredentialChecks(checks, provider, credentialEnvVars(provider, mapping.resource, connection), (envVar, index) => index === 0 ? envVarCheckId(provider) : `${envVarCheckId(provider)}.${envVarSuffix(envVar)}`);
        }
    }
    if (environments.length === 0) {
        ...
        for (const provider of PROVIDER_IDS) {
            addCredentialChecks(checks, provider, globalCredentialEnvVars(provider), (envVar, index) => index === 0 ? envVarCheckId(provider) : `${envVarCheckId(provider)}.${envVarSuffix(envVar)}`);
        }
    }
    for (const connection of listConnections(store)) {
        ...
        addCredentialChecks(checks, connection.provider, credentialEnvVars(connection.provider, undefined, connection), (envVar, index) => index === 0 ? `env.connection.${connection.id}` : `env.connection.${connection.id}.${envVarSuffix(envVar)}`);
    }
    try {
        store.appendAudit({
            timestamp: nowIso(),
...
export async function explainActionRisk(store, input) {
    assertProviderId(input.provider);
    assertCapability(input.capability);
    const project = resolveProject(store, input.project);
    const environment = resolveEnvironment(store, project, input.environment);
...
export function governedActionSummary(store, input = {}) {
...
function auditExportRows(entries) {
...
export function exportAuditLog(store, input) {
    const entries = listAuditLog(store, input);
...
export async function exportContextSnapshot(store, input) {
