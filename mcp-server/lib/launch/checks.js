 ...
 */
export function defaultProviderReads(store, plan) {
    ...
    return {
        ...
        dnsRecords: (domain) => pa.getDnsRecords(store, { ...base, domain }),
...
function evalError(detail) {
    return { satisfied: false, error: true, detail };
}
function evalResult(satisfied, detail) {
...
export async function evaluateRealityCheck(store, plan, step, reads) {
    ...
    switch (check.kind) {
        case "provider-mapped": {
            ...
            return evalResult(mapped, mapped
        ...
        case "domain-owned": {
            ...
            const owned = domains.some((d) => String(d?.name ?? "").toLowerCase() === domain.toLowerCase());
        ...
        case "dns-points-at-app": {
            ...
            const points = records.some((r) => {
                ...
                return (address === "76.76.21.21" ||
                    address === "vercel-dns.com" ||
                    address.endsWith(".vercel-dns.com"));
            });
            return evalResult(points, points
        ...
        case "stripe-webhook-enabled": {
            ...
            if (error)
                return evalError(error);
            const enabled = asArray(data, "data").some((w) => String(w?.status ?? "") === "enabled");
        ...
        case "env-var-present": {
            ...
            return evalResult(missing.length === 0, missing.length === 0 ? `App env has ${keys.join(", ")}.` : `App env is missing: ${missing.join(", ")}.`);
        }
        ...
        case "email-domain-verified": {
            ...
            if (error)
                return evalError(error);
            const domains = asArray(data, "data", "domains").map((d) => d);
            const match = domain ? domains.find((d) => String(d?.name ?? "").toLowerCase() === domain.toLowerCase()) : domains[0];
            if (!match)
                return evalResult(false, `Sending domain ${domain ?? ""} not found in Resend.`.trim());
            if (check.kind === "email-domain-exists")
                return evalResult(true, `Sending domain ${match.name} exists.`);
        ...
        default:
            return evalError(`Unknown reality check kind "${check.kind}".`);
    }
}
