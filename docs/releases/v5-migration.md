...
| **Capability registry CRUD/UI** (`/api/capabilities/{[id],access,health,history,test}`, `POST /api/capabilities`, `/api/agents/registry*`, `/api/agents/invoke`; SDK capabilities/agent-registry families; `dashclaw://capabilities`) | **The enforcement seam survives:** `dashclaw_invoke`, `GET /api/capabilities`, `/api/capabilities/[id]/invoke`, and `/api/capabilities/[id]/access/check` still enforce governance on invocation. Note: there is **no in-product create/edit path** post-cull — capabilities are seeded by direct SQL (inert-by-default). |
