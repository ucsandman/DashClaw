...
2. **Compile** — `compileMode(modeId)` turns a mode into an array of guard policies in exactly the shape `validatePolicy` accepts and `insertPolicy` stores. Every compiled policy:
   - uses one of the 19 live `policy_type` values (nothing is fabricated),
...
- **Catastrophe Only** — the **self-hosted default**, seeded automatically for every new org at its first migrate. Five catastrophe-only policies: **hold** mass-destructive operations (`rm -rf`, `DROP TABLE` — server risk clamps to 100), **hold** real-money spend (a registrar, card, checkout or credit top-up write, or a purchase CLI; ungrantable, so every purchase is its own exact-amount approval), **hold** secret-file writes (`.env`, `*.pem`, `*.key`, `secrets/**`) for one-click approval, **hold** force-pushes over protected branches (`main`, `master`, `trunk`, `production`, `release/*`), and a **warn-only** rate limit (200 actions / 10 min) that nets runaways without interrupting. Nothing is refused outright. Everything else runs. It holds writes to secret files, not reads (the Read tool is not hooked today). Import it at `/policies` to retrofit an org that predates the seed.
