...
- Known limitation (recorded, not hidden): the guard's operator-approval grant binds a retry to `agent_id` + exact `declared_goal` + `action_type` within a 15-minute single-use window — not to the approved action's parameters. A future release can bind on the action-content hash (`act_hash`) once the SDKs stamp it on both the pending record and the retry.
