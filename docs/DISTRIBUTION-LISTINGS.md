...
| Glama (glama.ai) | `mcp-server/glama.json` added (schema-verified; `maintainers: ["ucsandman"]`) + `mcp-server/Dockerfile` (build + credential-free introspection verified in Docker: `initialize` and `tools/list` answer with no env vars, 33 tools). **PR #9313's bot now requires a Glama listing + score badge to merge** — and Glama's "Add Server" flow is an authenticated web session, so the listing itself is accelerant #3 (below), now the PR's merge-blocker. [Status reply posted on the PR](https://github.com/punkpeye/awesome-mcp-servers/pull/9313#issuecomment-4885818236) with honest authorship; badge gets pushed to the PR branch once the listing exists. | **LIVE — submitted by Wes 2026-07-05 (Add Server, Server tab), approved same day.** Listing at https://glama.ai/mcp/servers/ucsandman/DashClaw. Same day, the maintainer drove Wes's authenticated Glama session (Chrome DevTools MCP, Wes logged in): **Glama release 0.1.0 published** (build spec: debian:trixie-slim, `npm install -g @dashclaw/mcp-server@2.0.1`, CMD `dashclaw-mcp`, dummy placeholders — server introspects credential-free; first build attempt failed on Glama's own infra pulling the base image, retry green), repo synced, `glama.json` moved to repo root (their checklist reads it there), profile 25%→67%. Score checklist: release ✓ glama.json ✓ license ✓ README ✓ author ✓. **Scan landed (verified 2026-07-05, later session): License A · Quality A · Maintenance A.** Badge pushed to the PR branch ([comment](https://github.com/punkpeye/awesome-mcp-servers/pull/9313#issuecomment-4885845180)); #9313 now waits only on the maintainer merge |
