...
| v3.5 | Attention budgets: approval-flood guard | DONE 2026-07-04 (v4.47.0; **the item's premise was false** — the W3 flood guard shipped complete in v4.15.0 (2026-06-12, in CHANGELOG) and the 2026-07-03 drafting sweep missed it; v3.5 became a closeout audit under the v3 truth bar, spec revision docs/superpowers/specs/2026-07-04-approval-flood-guard-revision.md. Audit verdicts: bulk already honors v2.3 expiry (sweep-before-match) and §1 holds structurally (pending_approval-only matching, protected_path refused); three June owner questions decided as maintainer — budget defaults KEPT (10/15m/30), digest default KEPT (24h, adapters-gated), pause-rule leaves pending KEPT (v2.3 expiry makes leftovers expire truthfully). The one real gap fixed: flood counting predated v3.1 and counted synthetic traffic — a smoke run could trip the fleet budget, suppressing REAL per-action pings org-wide + minting the red approval_flood signal; getRecentApprovalCountsByPolicy now excludes the shared synthetic predicate in-SQL, with an ephemeral ?include_synthetic=1 diagnostic view (never persisted/notified) as the smoke positive control. Smoke U1–U4 = 95/95; rendered proof: seeded non-synthetic 50-approval burst → ONE banner ("50 interrupts in 15m", armed pause/approve-all/deny-all), all 50 individually actionable, zero console errors, seeded rows cleaned + hysteresis cleared) |
| v3.6 | Enforcement over assertion | DONE 2026-07-04 (v4.48.0; spec docs/superpowers/specs/2026-07-04-enforcement-over-assertion.md. Defaults graduated on evidence — the live ledger's 176,149 guard decisions contained ZERO verified-JWT traffic, so both flips were free: DASHCLAW_JTI_REPLAY_PROTECTION best_effort→required (new shared getter app/lib/replay-protection.ts kills the duplicated default literal; store outage fails closed for verified traffic; API-key callers structurally exempt via not_applicable, now pinned by 7 new evaluateGuard-level mode tests closing the coverage gap act-binding never had) and DASHCLAW_ACT_BINDING off→best_effort (blocks only a PRESENT-claim mismatch — zero change for non-minting issuers; required stays opt-in with reason: it would gate JWKS adoption on claim minting); both keep one-env-var rollbacks per the v2.2/guard-fallback precedent. Enforcing proxy: recorded KILL in docs/architecture/enforcement-boundary.md (ADR) — consumer chat exposes no pre-execution interception point, MCP can offer but never wrap tools, and the capability-broker alternative was rejected as a product-shape change; the ADR carries the canonical per-surface mechanical-vs-cooperative table + supersession trigger (revisit if a consumer surface ships a hook contract). Truth pass applied across README/QUICK-START/PROJECT_DETAILS/runtime-api/SDK READMEs/governance skill/CLAUDE-DESKTOP-PLUGIN (which had NO advisory language at all)/marketing; constitution §1 intact — "blocks are absolute" scoped to the decision layer, never weakened. Human surface: /setup#enforcement-posture card reads the guard's own getters; in-ship security LOW fixed — weakened knobs render "review recommended" with value withheld on the public page. Gates 4998 green ×2; security review PASS 0H/0M; rendered proof both card states) |
| v3.7 | Deferred-debt triage | DONE 2026-07-04 (v4.49.0; spec docs/superpowers/specs/2026-07-04-deferred-debt-triage.md — every parked line has a written verdict. 9 BUILT: approvals expiry timestamps rendered+labeled (/approvals; list SELECT never carried approval_expires_at), degradation by_day strip rendered (/policies; fetched-never-rendered since v2.1), guard-load SLO calibrated 2000→1500ms from a warmed run (fast p99 444ms, record 735ms, no knee to 50 conns), x402 currency allow-list (DASHCLAW_X402_CURRENCIES, default USDC — junk currency summed 1:1 into USD budget ceilings), x402 idempotency_key (0047; the money route was the only sibling without retry dedup), apiErrorResponse production redaction (DASHCLAW_EXPOSE_ERROR_DETAIL; raw driver messages reached governed agents on 219 call sites), JWKS fail-closed when DASHCLAW_ALLOWED_ISSUER unset (any-reachable-JWKS-accepted let API-key holders forge verified identity; /setup posture card row added), Codex SessionStart digest parity (lifecycle VERIFIED by probing codex-cli 0.139.0 — event enum in binary + live registered hook; installer test pins it), Dependabot EOVERRIDE untangle (own pre-release commit fca42a34; duplicate postcss devDep removed, CVE override kept). Ride-along bug fixed: CRITICAL_TABLES_DDL fallback was stale pre-Phase-2 (guard_decisions missing 8 columns → 42703 hard-fail on the fallback branch); regenerated from schema.js + drift-gate test. 6 KILLED with reasons + revival triggers: /decisions risk hint (13-caller hot path), per-policy degradation split (timing artifact, not policy property), load CI wiring + LLM slow-path (flaky-by-nature; unseeded = "theatre"; revive on degradation recurrence), verification_status enum (single writer via TS union), per-org issuer binding (hosted multi-tenant future), assumption contradiction (no FP budget; similarity≠contradiction), calibration dup-detection + in-UI rename (rationale intact, zero recurrence)) |
...
> **Closeout correction (2026-07-04):** the premise below is false. The W3
> flood guard shipped complete in v4.15.0 (2026-06-12) — the drafting sweep
> searched this log and the roadmap ledger but not the CHANGELOG, where the
> ship is fully documented. The item executed as a closeout audit instead;
> verdicts in the ledger row above and the spec revision. Kept unedited
> below as the record of what the sweep believed.
