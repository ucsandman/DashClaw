...
1. **Per-org API keys cannot authenticate on self-hosted local Postgres** — `middleware.js resolveApiKey` (~line 385) hardcodes the Neon HTTP driver; only the `DASHCLAW_API_KEY` operator key works. Likely an edge-runtime constraint, but it contradicts the self-host + mint-keys story. Decide: node-runtime middleware, driver switch, or document operator-key-only for local Postgres.
...
3. **Wire-format quirk**: `POST /api/policies` requires `rules`/`agent_ids` as JSON strings and `active` as an integer — raw-HTTP integrators will trip on this; consider accepting objects/booleans too.
