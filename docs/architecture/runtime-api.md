...
`decision_id` is the canonical id of this guard evaluation (`act_gd_…`). Without successful inline recording, `action_id` is a deprecated alias of that decision id. Do not pass a decision id to approval or action endpoints. With `POST /api/guard?record=true`, require `recorded: true` and use the returned action-record `action_id`; a separate `POST /api/actions` can also create that record. Approval polling, execution claims, and outcome updates all require the action-record id. Since 5.35.0 a `?record=true` body may also carry `claim_execution: true` and an `attempt_id`; for an allow or warn verdict the server then performs the execution claim in the same request and echoes `claimed`, `attempt_id` and `claimed_at` (a refused claim is `claimed: false` with `claim_error: EXECUTION_CLAIM_CONFLICT`), so a governed call needs no follow-up `PATCH`. A response without a `claimed` key means the claim was not folded and the client sends the PATCH.
...
Two staging media, each with its own basis and its own capability string:

| Basis | Eligible act | Staging medium | Required capability |
|---|---|---|---|
| `file` | `Edit`/`Write`/`MultiEdit` file mutations | git worktree | `allow_contained` |
| `shell_file_ops` | a shell act the evidence classifier grades as a clean, non-network `apply` operation (no flags, not a `git push`/`pull`/`fetch`) | git worktree | `allow_contained` |
| `db_branch` | a `sql` act, or a shell act the classifier flags `database` carrying no other effect flag (`ddl` and `whereless` are NOT disqualifying — they are what containment is for) | ephemeral Neon branch of the target database | `allow_contained:db` |

The server has **no git-repository check and no Neon-account check at all**. Both requirements are enforced entirely **client-side, by the hook**: staging happens in a git worktree or a database branch, so outside a git repository — or with no `NEON_API_KEY`, or against a non-Neon host — the hook simply never advertises the capability for that call and the verdict lands as `require_approval` (RFC [2026-09-04](../rfcs/2026-09-04-database-containment.md)). HTTP calls, payments, message sends, and deploys are never eligible server-side.

**The database mechanism.** A Neon branch is copy-on-write, created in seconds, inherits its parent's roles and passwords, and self-deletes on an expiry timestamp — which is what makes a live database reversible enough to contain. On the first `db_branch` verdict of a session the hook branches the target database, rewrites the command's connection to the branch (URL substitution, or a `DATABASE_URL=…` prefix), and reuses that branch for the rest of the session exactly as it reuses one worktree. The evidence it posts is the statement as run, the schema diff Neon reports between the branch and its parent (or a note saying there is none — data changes are not diffable), and the tail of the command's output. Cleanup is expiry: nothing to remove, and Discard needs no Neon key.

**Capability negotiation is non-negotiable.** A caller only ever receives `allow_contained` if it advertised the capability string for **that basis** on the guard call (`context.client_capabilities`). A caller that does not advertise, or whose act fails the server's eligibility check, gets `require_approval` instead, with `risk_breakdown._containment: { downgraded_to_interrupt: true, reason: 'client capability not advertised' }` (the `reason` is the eligibility-check's basis string when eligibility itself failed, e.g. `'git network subcommand'`) — version skew only tightens, never loosens. In particular, a hook that advertises only `allow_contained` can never be handed a `db_branch` verdict. The bare Node and Python SDKs do not advertise either capability by default (a bare SDK caller has no worktree or branch machinery to stage into); only hook-cooperating harnesses (Claude Code, Codex, Hermes) that implement the redirect opt in.
...
**Database lifecycle differences.** A `db_branch` verdict stamps a `dashclaw/contained-db-<session>[-<instance>]` ref (same shape regex, the `db-` prefix inside the segment), and every consumer branches on that prefix. The evidence artifact is the same `artifact_type: 'patch'`, with `content.kind: 'db'` (`statement`, `diff`, `stdout_tail`, `branch_id`, `db_name`, `note`) — so the evidence binding, the card and the CLI all keep working. Promotion is a **replay, not a merge**: the synthetic grant's act is the action's *original recorded act* (read from the guard decision the row was created from, since `action_records` stores only its content hash), and its `risk_score` is the original action's score rather than the merge path's constant 20, because the replay IS the risky act. `dashclaw contained apply` runs that command through the operator's own shell and environment from the repo root, and cleans nothing up — the staged branch expires on its own. A db ref whose original act cannot be recovered refuses promotion with `409 CONTAINMENT_ACT_MISSING` rather than minting a merge grant for a branch that does not exist.
...
**Attestation (`attested_model`, `harness`, `harness_version`).** Every guard call from a hook-cooperating harness declares which model and which harness is making it. The same act is a different risk from a different model, and it is only governable at all on a harness carrying the hook — so the ledger now records *by what*, not only who and what. The Claude Code hook reads the model from the last assistant entry of the harness's own session transcript (`message.model`; it exists on neither hook stdin nor in the environment), reading only the file's tail on each call; Codex and Hermes declare `harness` and no model; a caller the hook cannot recognise says `unknown` rather than guessing. The three fields ride the persisted decision context (no schema change) and are lifted onto every reader — `GET /api/guard`, `GET /api/guard/decisions`, and the FK-linked `guard_decision` on `GET /api/actions/:id` — so the `/decisions` detail shows them as a chip. They are **client-declared attribution, never proof**: the transcript is a file the harness owns, so a hostile client can state anything. Same posture as `enforcement_mode`; no policy grants on these fields, and outward copy never claims DashClaw verifies which model acted.
