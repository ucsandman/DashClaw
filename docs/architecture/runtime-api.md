...
The canonical Node SDK is `dashclaw` on npm (version tracked in `sdk/package.json`). The canonical SDK file `sdk/dashclaw.js` exposes 82 public methods across the core runtime and extension surfaces (verify with `npm run sdk:count`).
