...
The server exposes **29 governance tools** across 12 groups — the core loop (`dashclaw_guard`, `dashclaw_record`, `dashclaw_wait_for_approval`, `dashclaw_assumption_record`), capability invocation (`dashclaw_invoke`), session lifecycle, handoffs, credential-rotation hygiene, skill safety scanning, open loops, learning and retrospection, the agent inbox, behavior-learning suggestions, governance posture, and work orders — plus 6 read-only resources (`dashclaw://policies`, `dashclaw://capabilities`, `dashclaw://agent/{agent_id}/history`, `dashclaw://status`, and the two code-sessions resources). The complete tool-by-tool table is [`mcp-server/README.md`](../../mcp-server/README.md).
