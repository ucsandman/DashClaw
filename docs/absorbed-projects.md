...
- `feedback` (`schema/schema.js:916-933`): full per-agent helpers in `app/lib/feedback.js` (rating, sentiment, `by_agent`), but every HTTP route is archived under `app/api/_archive/feedback/*`. Usable server-side via the lib or direct table read; `agent_id` is stored as `''` when absent.
