...
|---|---|
| Full v2 method catalogue (107 methods) + canonical HITL flow + error handling + Execution Studio usage | **[`sdk/README.md`](../sdk/README.md)** |
...
`GET /api/posture`, `GET /api/posture/findings`, `POST /api/posture/findings/:key/resolve`, and `POST /api/posture/scan` are read-only/operator-gated governance-posture routes — a gaming-resistant org score (6 dimensions) plus a human-gated remediation loop. They have **no SDK method** (and do not change the 107 Node / 183 Python counts); query them directly over HTTP, or use the `dashclaw_posture` / `dashclaw_posture_next` MCP tools or the `dashclaw posture` CLI command.
